...
before any is written.
    ...
    /usr/bin/python3 scripts/analyze_ft_robot_frame.py
    /usr/bin/python3 scripts/analyze_ft_robot_frame.py --excitation-run <dir>
...
def report_ft_frame(cfg: dict, cal, run: Path | None) -> dict:
    ...
    if not conv:
        ...
        print("     run scripts/manual_ft_excitation_test.py first")
        return {}
...
def main() -> int:
    ...
    (OUT / "frame_analysis.yaml").write_text(yaml.safe_dump({
        "generated_by": "scripts/analyze_ft_robot_frame.py",
