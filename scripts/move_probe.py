        ...
        while travel < a.max_travel:
            ...
            if mag >= a.contact_n:
                ...
                if contact_hits >= a.contact_hits:
                    print(f"\n  CONTACT at {mag:.3f} N, {h:.3f} mm above the "
                          "sensor plane")
                    print(f"  descended {travel:.3f} mm from the start")
                    outcome, surface = "contact", h
                    break
                print(f"    {mag:.3f} N over threshold "
                      f"({contact_hits}/{a.contact_hits})")
            else:
                contact_hits = 0
            if not fine and mag >= a.notice_n:
                fine = True
                print(f"  ({mag:.3f} N noticed — switching to {a.fine_step} mm steps)")
        else:
