...
print(f"  {'cell':14s}{'size':>6}  " + "  ".join(f"{p:>13s}" for p in srcs) + "   (median Fz MAE N per principle)")
for k, (h, t) in enumerate(cells):
    ...
    for pr in prs:
        ...
        for u, x in s.groupby("unit"):
            x = x.sort_values("width_px"); ax.plot([X[v] for v in x.width_px], x.fz, "-", color=COL[pr], alpha=.35, lw=1)
        med = s.groupby("width_px").fz.median().reindex(w)
        ax.plot([X[v] for v in w], med, "o-", color=COL[pr], lw=2.2, ms=5, label=f"{pr} (n={s.unit.nunique()})")
    ax.set_title(f"{HN[h]} {t} mm", fontsize=10.5); ax.set_ylim(0, None); ax.grid(alpha=.25)
    ...
    if len(prs) >= 2:
        for wv in (8, 48, 160, 854, 1920):
            if wv in w:
                vals = {pr: sel[(sel.principle == pr) & (sel.width_px == wv)].fz.median() for pr in srcs}
                print(f"  {HN[h]+' '+str(t)+'mm':14s}{wv:>6}  " + "  ".join(f"{vals.get(pr, np.nan):13.4f}" for pr in srcs))
for ax in axes[:, 0]: ax.set_ylabel("Fz MAE (N)")
fig.suptitle("Fz estimation error against input size, gel by gel — one panel per (hardness, thickness), one colour per principle", fontsize=12)
fig.tight_layout(rect=(0, 0, 1, 0.965)); out = os.path.join(ROOT, "docs", "figures", "cross_principle_fz.png"); fig.savefig(out, dpi=130); plt.close(fig)
print("  ->", out)
