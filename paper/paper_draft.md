...
[VI-1] Each cell of the design holds a single gel, so variation between nominally identical gels cannot be separated from the design factors. The cost shows in the spread between the nine specimens of one configuration: plateau densities range over two to three orders of magnitude (Table IV) and plateau shape errors over 2.6× and 3.8×, both wider than any systematic difference between the thickness or hardness groups, so the negative results on plateau density are limits of detection rather than evidence of independence, and only the positive trends — thickness in two-contact separation and in saturation depth — are large enough to clear that spread. The support condition also differs between the families, the depth-referenced gel sitting on a 4.5 mm compliant base and the photometric gel on acrylic, so the opposite thickness trends of the ceiling in Section IV-B may be a support effect rather than an imaging-principle effect; this design does not separate the two. All photometric specimens resolved the narrowest pair we could make (1.10 mm centre spacing), so their two-contact limit lies below that and was not measured, and the depth-referenced trend is entangled with seating symmetry, which was best on the 2 mm units. The ranges are narrow in places: hardness spans 40 Shore OO points in one family and 6 in the other, thickness spans 1 to 3 mm, and only two imaging principles and one camera were used, so the photometric hardness null results are indistinguishable from a range limitation; because a flat-punch measurement did not order the photometric grades consistently, "hardness" in that family should be read as a formulation label. Finally, area-averaging a 1920 × 1080 frame is not the same as a low-resolution camera, which has its own pixel size, noise, and optics — the plateau densities are those of this reduction, and Section IV-A shows one case where the full-resolution frame was the worse input — and the plateau density itself moved tenfold when the preprocessing scale was corrected (Section V-B). The resultant-force error is available only as a lower bound, the two highest force rungs were trained with one seed, the three configurations were trained on different input representations, absolute force accuracies are bounded by the label floor, and thickness changes magnification as well as mechanics.

[VI-2] The cheapest next experiment is to cast the depth-referenced gel on acrylic, as the photometric one is, and repeat the saturation ramp, which tests the support hypothesis with one family re-made. Beyond that, in order of cost: at least three specimens per condition, to separate casting repeatability from design effects; an absolute saturation criterion, so that ceilings compare across families; and narrower two-post probes with shallower ladder rungs, to find the photometric separation floor. For sensor builders, the campaign also identified factors beyond thickness and hardness that moved the numbers by amounts comparable to the design variables — seating tilt and the scale of preprocessing filters — so a gel described by its formulation alone is not reproducible to the level at which these trends were measured. We suggest reporting the two-contact dip at a stated depth alongside the formulation, which can be measured in a few minutes with a single probe.
