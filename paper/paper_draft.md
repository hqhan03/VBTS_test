...
**Fig. 1.** Where the two design variables of this study enter a sensor design. The user specification — (a) the target task and (b) the sensing principle — fixes the contact response the sensor must cover (in the example, a maximum force above 10 N) and the feature to be estimated with its allowable error (force estimation within 0.1 N). Those two are what (c) the gel, varied in thickness (top row) and in hardness (bottom row), and (d) the pixel density are chosen against before (e) the sensor is built; each panel shows the contact image its choice produces, and the frames in (d) are R = 220, 13.8, and 0.55 px/mm^2. The boxed cells are the choice carried through the example. The values are illustrative, and the arrows are the order in which the questions arise in this study, not a validated design procedure.
...
[IV-B4] Repeating the decision on frames area-averaged to the twelve widths of Section V (Fig. 4e) separates three outcomes: resolved; contact detected but not separated; and contact not detected at all. For the 2.00 mm pair the resolved count peaked at 19 of 27 rungs between R ≈ 390 and 1600 px/mm^2, fell to 11 at R ≈ 14, and dropped to 12 at full resolution, where per-pixel noise was five times higher and the median imprint fell below the signal gate. Below R ≈ 200 a growing share of the failures are contacts the detector never found, so values there bound the failure rather than measure it. This decision therefore uses pixels one to two orders of magnitude above the force and shape plateaus of the same camera, and full resolution is not its best operating point.
...
22. **A paragraph tag leaked into the body:** "[III-B4] Not every measurement was made…" on page 3. The tags in this file are working marks and must come off when the text is pasted.
