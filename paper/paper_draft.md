...
12. **Page budget.** Sections I–VII are about 6,900 words (tables and captions excluded), plus seven tables and three figures; at roughly 1,000 words per two-column page this is about 11 pages against a limit of 8 including references, so roughly 3,000 more words must come out. Beyond further paragraph trimming, the options are to fold Table VII into the text, drop the 5 % and 20 % columns of Table VI (see item 14), and decide whether the marker configuration and Section IV-B stay. Recommended order of cuts: (a) merge IV-A4 into IV-A3 and drop the second half of IV-B3 (asymmetry) into one sentence; (b) shorten IV-B4 to the three-class result and the full-resolution loss (drop the per-width counts); (c) compress III-C3/C4 to protocol essentials; (d) drop Table III's brightness columns or fold Table III into text; (e) drop the 5 %/20 % columns of Table VI into one sentence; (f) shorten V-B2 (pipeline description) and V-B5 (lateral size); (g) merge VI-A into three paragraphs. Do not cut IV-B3's optimal-depth finding or V-B4's preprocessing-scale finding.
13. **Resultant force is now measured upstream.** `result/results_single_sensor.md` records a 2026-09-14 retrain that produced the actual E‖ΔF‖. The lower bound held in 99–100 % of cells but ran at 0.86–0.90× the true value, i.e. it located the plateau correctly and understated the magnitude by about 10 %. Section V-A5 still presents only the bound and should be rewritten against the measured curve.
14. **The 5 % tolerance result is retracted upstream.** The same document shows the 5 % plateau is not reproducible between two training runs of the same nine specimens: the density moves by up to 3×, and which configuration degrades fastest reverses between runs. It instructs that only the 10 % plateau be cited. Table VI's 5 % column, the V-A4 (iii) sentence, and the abstract's "from 5 % to 20 %" clause all rest on the retracted result and must be revised.
15. **Figure pipeline moved.** `CLAUDE.md` §2 places paper figures and their code in `paper/fin_figures/` (built through `paper_style.py`, one-column 3.50 in, two-column 7.16 in) and says not to mix them with `paper/figures/`. The captions in this draft still point at `paper/figures/*.pdf`, and `paper/figure_plan.md` is the authority on which figures are carried. Reconcile the caption file names and the figure list before submission.
