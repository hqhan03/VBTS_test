...
**Fig. 1.** Where the two design variables of this study enter a sensor design. The user specification — (a) target task, (b) sensing principle — fixes the contact response the sensor must cover and the feature to be estimated; those fix an allowable force range and an allowable error, which are what (c) the gel and (d) the pixel density are chosen against. In (d) one frame is shown at R = 220, 13.8, and 0.55 px/mm^2. The arrows are the order in which the questions arise in this study, not a validated design procedure.
