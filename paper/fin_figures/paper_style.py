...
> **ΔE 7.4 (deutan)** 다 — Okabe-Ito 셋의 11.0 보다 낮고, 검증기가 "보조 표시
> (직접 라벨·모양·질감)가 있을 때만 쓸 수 있다" 고 적는 6 ~ 8 구간이다. 보통
> 시야에서는 28.8 로 넉넉하고 바탕 대비도 셋 다 3:1 을 넘는다.
>
> 지금 그림들은 **표식 모양을 쓰지 않으므로**(운전자 결정) 그 보조 표시가 없다.
> 되살리려면 둘 중 하나면 된다 — 선 끝에 직접 라벨을 달거나, 선 모양을
> (실선·파선·점선) 갈라 주는 것. 흑백 인쇄에서도 그 둘이 답이다.
"""
import sys
...
# 투박한 넷. 순서가 있는 변수는 **차가운 쪽에서 뜨거운 쪽으로** 간다 —
# 무를수록 파랑, 단단할수록 빨강. 두께도 같은 차례다.
