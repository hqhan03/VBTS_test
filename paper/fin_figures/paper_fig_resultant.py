...
**그런데 두 선이 다른 학습 판에서 왔다** (2026-09-15 확인). `resultant_force.py`
가 하한·상한은 `force_vs_resolution_axes.csv`(판 A)에서 만들고 실측 합력만
`force_vs_resolution_res.csv`(판 B)에서 읽었다. **부등식은 같은 예측 위에서만
성립하므로 이 그림의 파선은 실선의 하한이 아니다** — 두 학습을 나란히 놓은 것에
가깝다. Marker 160 px 에서 파선이 실선 위로 올라가는 것이 그 증거다.

두 판은 학습의 확률성만 다르지만 그 차이가 작지 않다 — 5 % 평탄점이 최대 세 배
어긋난다(`results_single_sensor.md` 의 A·B 표). 그러므로 **위의 "0.86 ~ 0.90 배"
와 "35/36" 은 부등식 검사가 아니라 두 판의 비교로 읽어야 한다.**

**고칠 수 있다.** 판 B 의 csv 도 `fx_mae`·`fy_mae`·`fz_mae` 를 함께 담으므로
같은 판에서 하한을 만들 수 있다. `resultant_force.py` 에 `axes_source()` 를 두어
그렇게 하도록 고쳤지만, 원자료가 `data/` 에 있어 **실험 기계에서 다시 돌려야**
이 그림의 하한이 진짜 하한이 된다. 그때까지 캡션은 위의 단서를 달고 나간다.
