...
**겔은 힘의 무릎을 움직이지 않는다.** 위 **12 개 검정**(행 6 개 ×
...
**같은 칸의 두 복제가 내는 무릎이 얼마나 다른가**가 이 검정의 한계를 정한다:
...
**겔은 형상의 무릎을 움직이지 않는다.** 위 **8 개 검정**(행 4 개 ×
두께·경도) 어느 것도 q < 0.05 가 아니다. q 는 **힘과 형상을 합친 20 개**에
Benjamini-Hochberg 를 건 값이다 — 두 절을 따로 보정하면 "20 번 중 하나" 라는
사실이 가려진다.
...
| 원리 · 지표 | 쌍 | 배율 중앙 | 같은 단 | 8 배 이상 |
|---|---:|---:|---:|---:|
| 9DTact · cyl4 | 8 | **1.0×** | 5 | 0 |
| 9DTact · cube4 | 8 | **2.0×** | 3 | 0 |
| DIGIT · cyl4 | 9 | **1.7×** | 4 | 1 |
| DIGIT · cube4 | 9 | **1.0×** | 5 | 1 |

**형상은 사정이 낫다** — 네 조합 중 셋에서 복제 절반 이상이 **같은 단**에
떨어진다(9DTact cyl4 는 여덟 쌍 중 다섯이 정확히 같다). 재현되는 지표에서도
두께·경도 효과가 보이지 않으므로, 적어도 형상에서는 **정말로 없다**는 쪽에
무게가 실린다. 힘 쪽(8 절)은 그렇게 말할 수 없다.
