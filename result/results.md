...
| 원리 | 잰 것 | 경도별 폭 | 두께별 폭 | 두께가 단조인가 | 경도 비 | 두께 비 |
|---|---|---:|---:|---|---:|---:|
| 9DTact | 자국 지름 | 5 % | **13 %** | **아니오** | 0.41 | **10.35** |
| 9DTact | 밝기 변화 | 12 % | **65 %** | **아니오** | 0.46 | **6.44** |
| DIGIT | 자국 지름 | 6 % | **21 %** | 예 | 0.76 | **2.92** |
| DIGIT | 밝기 변화 | 9 % | **35 %** | 예 | 0.41 | **2.54** |
| DIGIT_Marker | 자국 지름 | 10 % | **19 %** | 예 | 0.65 | **2.16** |
| DIGIT_Marker | 밝기 변화 | 12 % | **36 %** | 예 | 0.38 | **3.87** |

<sub>**비** = 군 사이 차이 ÷ 군 안의 사분위 폭. 1 보다 작으면 그 군들은 자기
흩어짐 안에 있다는 뜻이다. 자료: `<원리>/data/optical_group_spread_<프로브>.csv` · 곡선 `optical_by_group_<프로브>.csv`</sub>

**비가 판정한다.** 두께별은 여섯 칸 모두 **2.2 ~ 10.4** 로 1 을 넘고, 경도별은
여섯 칸 모두 **0.38 ~ 0.76** 으로 1 에 못 미친다. 원리와 지표를 가리지 않고
갈라진다 — **두께는 유닛 산포 위로 올라오고 경도는 그 안에 묻힌다.**

가는 선을 보면 왜 폭만으로는 부족한지 알 수 있다. 중앙값 세 줄은 깔끔하게
갈라져 보여도 유닛 하나하나는 군을 넘나든다 — 특히 9DTact 의 경도별 지름은
군 사이 22 px 에 군 안 IQR 54 px 이라, 중앙값의 차이가 **한 유닛 고르는 것보다
작다.**
...
### 깊이 기준과 힘 기준이 서로 다른 답을 준다

2 절과 같은 방식으로 **군 사이 차이 ÷ 군 안의 사분위 폭**을 잰다. 1 보다 크면
그 변수가 유닛 산포 위로 올라온다는 뜻이다.

| 원리 | 잰 것 | 나눈 것 | 깊이 기준 비 | **힘 기준 비** |
|---|---|---|---:|---:|
| 9DTact | 자국 지름 | 경도 | 0.41 | **3.85** |
| 9DTact | 자국 지름 | 두께 | 10.35 | 0.38 |
| 9DTact | 밝기 변화 | 경도 | 0.46 | **1.91** |
| 9DTact | 밝기 변화 | 두께 | 6.44 | **1.03** |
| DIGIT | 자국 지름 | 경도 | 0.76 | 0.65 |
| DIGIT | 자국 지름 | 두께 | 2.92 | **2.82** |
| DIGIT | 밝기 변화 | 경도 | 0.41 | **1.28** |
| DIGIT | 밝기 변화 | 두께 | 2.54 | **2.03** |
| DIGIT_Marker | 자국 지름 | 경도 | 0.65 | 0.76 |
| DIGIT_Marker | 자국 지름 | 두께 | 2.16 | **2.46** |
| DIGIT_Marker | 밝기 변화 | 경도 | 0.38 | **1.33** |
| DIGIT_Marker | 밝기 변화 | 두께 | 3.87 | **2.86** |

<sub>자료: `<원리>/data/force_group_spread_<프로브>.csv` · 곡선 `force_by_group_<프로브>.csv`</sub>

**9DTact 에서 정확히 뒤집힌다.** 자국 지름을 깊이에 대해 보면 두께가 갈리고
(비 10.35) 경도는 묻히는데(0.41), 힘에 대해 보면 **경도가 갈리고(3.85) 두께가
사라진다(0.38).** 같은 자국, 같은 램프, 바뀐 것은 x 축뿐이다.
...
> **DIGIT 계열에서는 뒤집히지 않는다.** 두께가 두 기준 모두에서 갈리고(2.0 ~ 2.9)
> 경도는 힘 기준에서 겨우 1.3 까지만 올라온다. 9DTact 는 경도를 40 Shore 점
