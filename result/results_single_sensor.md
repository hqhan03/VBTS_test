...
![Figure 5 — 한 카메라에서 두 과제가 요구하는 화소 밀도](../paper/figures/fig_task_requirement.png)

같은 9DTact 카메라에서 힘과 형상을 같은 x 축에 놓는다. 위 섬네일이 그 밀도에서 영상이 실제로 어떻게 보이는지다. **▲ 는 평탄 구간이 시작하는 곳**(유닛별 평탄점의 중앙), **✕ 는 평탄 오차의 두 배를 넘어 못 쓰게 되는 곳**이다. 띠는 아홉 유닛의 사분위 범위 — 불확실성이 아니라 유닛 사이의 산포다. 힘 쪽에 Fz 가 아니라 **합력**을 그린 이유는 아래에 적었다.
...
| DIGIT | `hard_1mm_r1` · `hard_2mm_r2` · `hard_3mm_r1` · `medium_1mm_r1` · `medium_2mm_r1` · `medium_3mm_r1` · `soft_1mm_r2` · `soft_2mm_r1` · `soft_3mm_r1` |
...
| DIGIT | ρ +0.05 (p 0.893) | **ρ -0.90 (p 0.001)** | ρ -0.05 (p 0.893) | **ρ +0.95 (p 0.000)** |
...
### 센서마다 — 어느 밀도에서 어느 간격까지

위 그림은 간격 하나(2.00 mm)를 17 유닛 평균으로 본 것이다. **간격을 전부**
(1.10 · 1.25 · 1.50 · 1.75 · 2.00 · 2.50 mm) 쓸어 센서마다 한 판으로 놓으면,
"이 센서는 이 밀도에서 여기까지 가른다" 를 직접 읽을 수 있다.

파란 점이 그 칸에서 갈린 깊이 단이 있다는 뜻이고 **크기가 몇 단인지**를
나타낸다 — 작은 점은 한 단만 간신히 갈린 것이다. 회색 ×는 한 단도 못 갈린
칸이다. 주황 계단이 그 밀도에서 **분해되는 가장 좁은 간격**이고, 아래로
내려갈수록 좋다.

![9DTact — 센서마다, 어느 화소 밀도에서 어느 중심 간격까지 분해되나 (여섯 간격 전부)](single/extra/figures/K_spatial_sweep_9DTact.png)

*9DTact — 센서마다, 어느 화소 밀도에서 어느 중심 간격까지 분해되나 (여섯 간격 전부)*

<sub>그림: `single/extra/figures/K_spatial_sweep_9DTact.png` · 자료: `single/extra/data/K_spatial_sweep_9DTact.csv`</sub>

![DIGIT — 같은 그림. 간격은 1.10 과 1.25 mm 두 가지만 쟀다](single/extra/figures/K_spatial_sweep_DIGIT.png)

*DIGIT — 같은 그림. 간격은 1.10 과 1.25 mm 두 가지만 쟀다*

<sub>그림: `single/extra/figures/K_spatial_sweep_DIGIT.png` · 자료: `single/extra/data/K_spatial_sweep_DIGIT.csv`</sub>

**여섯 간격이 모두 갈리는 것은 R ≳ 220 부터다.** 그 아래에서는 3 ~ 5 개만 갈린다. 9 절의 스윕이 분해 비율로 본 것과 같은 자리인데, 이 그림은 **어느 간격이** 살아남는지까지 보여 준다 — 넓은 간격부터 남고 좁은 쪽이 먼저 죽는다.

**가장 좁게 갈린 간격은 유닛마다 다르다** — 두께별 중앙이 1 mm 겔 1.25 mm, 2 mm 겔 1.10 mm, 3 mm 겔 1.50 mm 다. 3 mm 겔이 가장 나쁘다.

> 다만 **검정은 통과하지 못한다** — 두께와의 Spearman ρ +0.477 (p 0.19), 경도는 ρ +0.168 (p 0.67), n = 9. 방향은 4 절의 대비 이야기와 맞지만(두꺼운 겔이 두 자국을 뭉갠다) 이 아홉 유닛으로는 크기를 주장할 수 없다.

> **한 단이라도 갈리면 분해로 센다.** 묻는 것이 "이 간격을 가를 수 있는가"
> 이지 "모든 깊이에서 가르는가" 가 아니기 때문이다. 그래서 작은 점은
> 약한 증거다 — 계단선이 작은 점 하나에 끌려 내려간 구간은 믿지 말 것.

> **DIGIT_Marker 는 없다** — 공간 분해능을 재지 않았다(2026-09-11 결정).
> DIGIT 은 `pair010`·`pair025` 만 있어 세로축이 두 칸뿐이다.

<sub>만드는 것은 `scripts/spatial_sweep_units.py` · 자료 `extra/data/K_spatial_sweep_*.csv`, 원자료 `extra/data/resolution_sweep_<원리>_<간격>.csv`</sub>
...
### 마커 팔의 전처리 — 지운 것과 남긴 것

머리 결과의 마커 팔은 `inpaint`, 곧 마커 점을 **지운** 표현이다.
그런데 마커 기반 센서의 표준은 Marker Displacement Method 로 마커를
**추적**해 전단을 읽고, FEATS 는 GelSight Mini 원본 이미지를 그대로
넣는다. 우리는 그 신호를 지우고 학습했다(2026-09-14 운전자 지적).

그래서 마커를 남긴 `colour`·`grey` 로 선택 아홉 유닛을 축분리로 다시
학습했다. **같은 프레임, 같은 설정, 같은 학습 판이고 입력 표현만**
**다르다.**

![DIGIT_Marker — 세 전처리를 나란히. 전단 · Fz · 합력.](single/3_DIGIT_Marker/figures/marker_reps.png)

*DIGIT_Marker — 세 전처리를 나란히. 전단 · Fz · 합력.*

<sub>그림: `single/3_DIGIT_Marker/figures/marker_reps.png` · 자료: `single/3_DIGIT_Marker/data/marker_reps.csv`</sub>

**세 곡선이 거의 겹친다.** 전해상도 한 점에서 짝지으면 차이가 없다는
`force_estimation.md` §7.3b 의 결과가 해상도를 가로질러도 유지된다.

차이가 나는 곳은 **곡선의 오른쪽 끝**이다. 1920 px 가 80 px 보다 나은
유닛과 그 비(1920÷80):

| 전처리 | 나은 유닛 | p | 전단 비 |
|---|---:|---:|---:|
| `inpaint` (지움) | 7/9 | **0.039** | **0.85** |
| `colour` (남김) | 7/9 | 0.359 | 0.95 |
| `grey` (남김) | 6/9 | 0.301 | 0.94 |

**방향은 살아남고 세기는 살아남지 않는다.** 마커 없는 두 팔은
전해상도에서 34 ~ 36 % 나빠지는데(비 1.34 · 1.36) 마커 팔은 어느
전처리에서도 나빠지지 않는다. 다만 지우면 15 % 좋아지고 남기면
사실상 평평하다.

> 그러므로 정확한 진술은 **"마커는 전단이 고해상도에서 나빠지는 것을
> 막는다"** 이지 "고해상도에서 좋아지게 한다" 가 아니다. 뒤의 문장은
> `inpaint` 에서만 성립한다. `cross_principle.md` §3.5a.

<sub>만드는 것은 `scripts/marker_reps.py` · 자료 `3_DIGIT_Marker/data/marker_reps.csv`, 원자료 `reps_*.csv`</sub>

### 겔 설계에 따른 MAE 와 R²

여기까지는 해상도가 가로축이었다. 이 절은 **해상도를 없애고** 겔만 본다 —
유닛마다 평탄 구간(가로 80 px 이상)의 중앙값을 취해 수 하나로 만든 뒤,
두께와 경도로 가른다.

**MAE 와 R² 를 함께 보는 이유**는 둘이 갈릴 수 있기 때문이다. MAE 는
"얼마나 틀리나", R² 는 "신호를 실제로 따라가나" 다. 범위가 좁은 유닛은
MAE 가 작아도 R² 가 낮고, 배율만 틀린 유닛은 R² 가 높아도 MAE 가 크다 —
형상 쪽에서 이미 본 것이다(상관 0.99 인데 깊이가 참값의 0.61 배).

**선은 다른 인자를 고정한 것**이다. 가로가 두께면 선 셋은 soft·medium·
hard 각각의 두께 곡선이고, 가로가 경도면 1·2·3 mm 각각의 경도 곡선이다.
아홉 유닛이 3 × 3 이라 칸마다 하나씩이므로 **선마다 점이 정확히 셋**이다.
선이 나란하면 두 인자가 더해지는 것이고, **엇갈리면 상호작용**이다.

![힘 추정 — 겔 설계에 따른 Fz MAE 와 R². 점 하나가 유닛 하나이고, 선은 다른 인자를 고정한 것이다.](single/extra/figures/J_gel_effect_force.png)

*힘 추정 — 겔 설계에 따른 Fz MAE 와 R². 점 하나가 유닛 하나이고, 선은 다른 인자를 고정한 것이다.*

<sub>그림: `single/extra/figures/J_gel_effect_force.png` · 자료: `single/extra/data/J_gel_effect_force.csv`</sub>

![형상 복원 — 겔 설계에 따른 원기둥 MAE 와 R². 점 하나가 유닛 하나이고, 선은 다른 인자를 고정한 것이다.](single/extra/figures/J_gel_effect_shape.png)

*형상 복원 — 겔 설계에 따른 원기둥 MAE 와 R². 점 하나가 유닛 하나이고, 선은 다른 인자를 고정한 것이다.*

<sub>그림: `single/extra/figures/J_gel_effect_shape.png` · 자료: `single/extra/data/J_gel_effect_shape.csv`</sub>

**검정 40 개 중 p < 0.05 가 9 개**다. 유닛을 단위로 한
Spearman ρ 이고, 두께·경도는 세 수준, 원리당 유닛은 아홉이다.

**전부 두께다.** 두께 20 개 중 9 개가 p < 0.05 인데 경도는 20 개 중 0 개다. |ρ| 의 중앙도 두께 0.55 대 경도 0.18 로 갈린다. 보정을 하지 않은 표라 개별 칸은
믿을 수 없지만, **두께 쪽에만 몰리는 이 치우침 자체**는 우연으로 보기
어렵다 — 경도가 0/20 이면 경도 효과가 있다는 증거는 이 자료에 없다.

> 4 절이 겔 범위를 적어 둔 것과 맞는다 — 두께는 1·2·3 mm 로 세 배가
> 벌어지는데 경도는 실측 쇼어 A 로 두 단계가 겹친다. **설계한 폭이 다르면
> 검출력도 다르다.**

| 과제 | 원리 | 지표 | 인자 | ρ | p | 1/soft | 2/medium | 3/hard |
|---|---|---|---|---:|---:|---:|---:|---:|
| 힘 | 9DTact | 전단 MAE (N) | 두께 | **+0.738** | 0.023 | 0.0251 | 0.028 | 0.0373 |
| 힘 | DIGIT | Fz MAE (N) | 두께 | **-0.738** | 0.023 | 0.0854 | 0.0634 | 0.0611 |
| 형상 | 9DTact | 정육면체 MAE (mm) | 두께 | **-0.738** | 0.023 | 0.1238 | 0.0768 | 0.0618 |
| 힘 | DIGIT_Marker | Fz MAE (N) | 두께 | **-0.738** | 0.023 | 0.1313 | 0.0814 | 0.0734 |
| 형상 | 9DTact | 정육면체 R² | 두께 | **+0.738** | 0.023 | 0.4506 | 0.7258 | 0.8305 |
| 힘 | 9DTact | Fz MAE (N) | 두께 | **+0.685** | 0.042 | 0.0498 | 0.0434 | 0.0536 |
| 힘 | 9DTact | 횡력 크기 R² | 두께 | **-0.685** | 0.042 | 0.9525 | 0.9432 | 0.9318 |
| 힘 | DIGIT_Marker | Fz R² | 두께 | **+0.685** | 0.042 | 0.9229 | 0.9412 | 0.9592 |
| 형상 | DIGIT | 원기둥 MAE (mm) | 두께 | **+0.685** | 0.042 | 0.0421 | 0.0488 | 0.0837 |

> **이 표를 결과로 읽지 말 것.** 세 가지 이유다.
>
> 1. **다중검정 보정을 하지 않았다.** 40 번 검정하면 우연히
>    2 개쯤이 p < 0.05 로 나온다 — 지금 9 개다.
>    5 절의 포화 해상도 검정(20 개 BH 보정)과는 **다른 집합**이고, 그
>    보정에 합치지 않았다.
> 2. **n = 9 에 수준이 셋**이다. Spearman ρ 가 ±0.738 이면 p ≈ 0.023 인데,
>    유닛 하나가 순위를 바꾸면 그 값이 크게 움직인다.
> 3. **같은 방향이 두 원리에서 나오지 않는다.** 아래에 적는다.

**형상에서 두 원리의 부호가 갈린다:**

| 지표 | 인자 | 9DTact ρ | DIGIT ρ |
|---|---|---:|---:|
| 원기둥 MAE (mm) | 경도 | +0.158 | -0.580 |
| 원기둥 MAE (mm) | 두께 | -0.053 | +0.685 |
| 원기둥 R² | 경도 | -0.105 | +0.474 |

두 파이프라인이 같은 겔을 반대로 읽는다면, 그것은 겔의 성질이
아니라 파이프라인의 성질이다.

**선이 나란한 것은 힘의 두께 쪽뿐이다.** 다른 인자를 고정하고 잰 기울기의
부호가 셋 다 같은지 세어 보면, Fz 는 세 원리 모두 **두께에 대해** 부호가
같다(MAE 는 9DTact +, DIGIT·마커 −). 그런데 **경도에 대해서는** 9DTact 와
마커만 같고 DIGIT 은 엇갈린다. 형상은 더 나쁘다 — 9DTact 는 네 조합 모두
엇갈리고, DIGIT 은 MAE 만 같고 R² 는 엇갈린다.

> **엇갈린다는 것은 ρ 하나로 요약하면 안 된다는 뜻이다.** 위 표의 Spearman
> 은 아홉 점을 한 줄에 세워 재는데, 선이 엇갈리면 그 한 줄이 존재하지
> 않는다. 그래서 이 절의 결론은 **"두께 쪽에 신호가 몰린다"** 까지이고,
> 칸별 크기를 인용할 근거는 되지 못한다.

**R² 가 음수인 칸이 있다.** DIGIT 정육면체 R² 가 1 mm 와 3 mm 겔에서 0 아래다 — 그 유닛들에서는 **참 깊이의 평균을
그냥 내놓는 편이 복원보다 낫다**는 뜻이다. MAE 만 보면 0.07 ~ 0.10 mm 로
나쁘지 않아 보이는데, 깊이가 변해도 예측이 따라가지 않는다. **MAE 와
R² 를 함께 보라는 것이 바로 이런 경우다.**

<sub>만드는 것은 `scripts/gel_effect.py` · 자료 `extra/data/J_gel_effect_{force,shape}.csv`, 검정 `..._tests.csv`</sub>
...
> **2026-09-14 에 다시 학습해 실측값을 얻었다.** 아래 그림의 굵은 선이
> 실측이고 점선이 하한, 띠가 부등식의 폭이다. 하한은 99 ~ 100 % 의 칸에서
> 성립했고 참값의 **0.86 ~ 0.90 배**였다 — 평탄점의 위치는 같게 짚었지만
> 크기를 10 % 쯤 낮게 잡고 있었다. 아래 단락은 그 하한을 쓰던 때의 설명이고,
> 부등식이 왜 필요했는지를 남겨 둔다.
...
|---|---:|---:|---:|---:|
| 9DTact | **0.0628** | 80 px | 2.18 | 63 % |
| DIGIT | **0.0778** | 80 px | 2.93 | 69 % |
| DIGIT_Marker | **0.0967** | 160 px | 90.3 | 78 % |

**합력 오차의 바닥은 원리 순서를 그대로 따른다** — 9DTact 0.063, DIGIT 0.078, DIGIT_Marker 0.097 N. 세 곡선 모두
R ≈ 10 아래에서 가파르게 나빠지고 그 위로는 평평하다.

**합력은 사실상 Fz 다.** 제곱합에서 Fz 의 몫이 9DTact 63 %, DIGIT 69 %, DIGIT_Marker 78 % 로 셋 다 등분선
...
**실무적으로**: 세 원리 모두 합력 오차 0.06 ~ 0.10 N
이고 측정 범위가 0 ~ 2.1 N 이므로 **범위의 3 ~ 5 %** 다. 그 값을
더 낮추려면 해상도가 아니라 Fz 를 고쳐야 한다.

### 허용치를 조이면 답이 재현되지 않는다

**같은 아홉 시편을 같은 설정으로 두 번 학습했다.** 2026-09-12 판(A)과
합력을 얻으려고 돌린 2026-09-14 판(B)이고, 다른 것은 학습의 확률성뿐이다.
평탄 밀도를 세 허용치로 재면:
...
| DIGIT_Marker | A | 8.1 | 23 | 27 | 1.2× |
| DIGIT_Marker | B | 8.1 | 22.4 | 71.7 | 3.2× |

**20 % 와 10 % 는 사다리 한 단 안에서 맞는다. 5 % 는 맞지 않는다** — 밀도가
최대 세 배 움직이고, **어느 계열이 더 빨리 나빠지는지의 순서가 뒤바뀐다.**
A 판에서는 깊이 참조형이 1.0 배로 가장 단단하고 광도 스테레오가 2.8 배로
취약했는데, B 판에서는 정확히 반대다(3.3 배 대 1.4 배).

**10 % 에서도 Fz 혼자는 재현되지 않는다.** 위 표는 합력 기준이다. 같은 두
판을 지표별로 보면:

| 지표 (9DTact) | 판 A | 판 B | 배율 |
|---|---:|---:|---:|
| Fz | 17.3 | **44.7** | **2.6×** |
| 전단 | 5.9 | 6.2 | 1.05× |
| 합력 | 13.6 | 13.6 | 1.00× |

**합력이 Fz 에 끌려가는데도**(제곱합의 59 %) **Fz 보다 안정적이다.** 전단
항이 더해지면서 최솟값이 칼날 위에 서 있지 않게 되기 때문으로 보이는데,
사후 설명이고 검정한 것은 아니다. 실무적으로는 **인용할 한 수가 필요하면
Fz 가 아니라 합력을 쓰라**는 뜻이다 — Figure 5 의 힘 패널이 합력인 이유다.

> **왜 5 % 만 이런가.** 평탄점은 "그 시편 최솟값의 105 % 안에 드는 가장
> 낮은 단" 이다. 그 최솟값 자체가 시끄러운 argmin 이라, 문턱을 조일수록
> 통과 집합이 작아지고 seed 하나에 좌우된다. 110 % 는 통과 집합이 넓어
> 그렇지 않다.

> **그래서 철회한다.** 이 문서와 원고가 한때 "허용치를 5 % 로 조이면 깊이
> 참조형 수직력이 열두 배 오른다" 고 적었다. 그것은 판 하나의 성질이었고
> 두 번째 판에서 살아남지 못한다. **10 % 평탄점만 인용한다.**
...
<sub>유닛 9 개 — 두께 ρ -0.527 (p 0.145, q 0.579) · 경도 ρ +0.211 (p 0.586, q 0.893)</sub>
...
<sub>유닛 9 개 — 두께 ρ -0.158 (p 0.685, q 0.893) · 경도 ρ -0.369 (p 0.329, q 0.821)</sub>
...
<sub>유닛 9 개 — 두께 ρ -0.580 (p 0.102, q 0.509) · 경도 ρ +0.053 (p 0.893, q 0.893)</sub>
...
<sub>유닛 9 개 — 두께 ρ -0.580 (p 0.102, q 0.509) · 경도 ρ +0.105 (p 0.787, q 0.893)</sub>
...
<sub>유닛 9 개 — 두께 ρ +0.422 (p 0.258, q 0.738) · 경도 ρ +0.053 (p 0.893, q 0.893)</sub>
