...
행이 감지 원리, 열이 Fz 와 전단. 가는 선은 시편별 seed 중앙값, 굵은 선은 두께별 시편 중앙값, 축 아래 삼각형이 평탄 구간의 시작이다. **두께별 굵은 선 셋이 가는 선들의 산포 안에 들어 있다.**
...
<sub>만드는 것은 `src/scripts/paper_fig1.py` ~ `paper_fig4.py`. Figure 2·3 은 복제 선택 규칙으로 고른 시편 9 개를 쓴다 — 이 문서와 같은 집합이다.</sub>
...
**공간 분해능은 힘·형상보다 훨씬 많은 픽셀을 쓴다.** 분해 비율이 426×240 에서
최대(71 %)이고 320×180 아래로 무너진다. 골 깊이가 Rayleigh 를 넘는 것은
80×45 까지이고 48×27 에서 0.20 으로 내려간다. 힘은 48×27, 형상은 80×45 ~
160×90 에서 포화하므로 **같은 카메라에서 세 과제가 서로 다른 픽셀을 요구한다.**

**전해상도가 여기서도 손해다.** 1920×1080 의 분해 비율이 39 % 로 426×240 의
71 % 보다 낮다. 골 자체는 얕아지지 않는데(0.384 대 0.464) **픽셀당 잡음이
네 배**여서(0.108 대 0.026) "골이 자기 잡음의 3 배" 관문이 걸린다. 자국
밝기도 2.15 로 판정 바닥 2.5 아래다.

> **낮은 쪽 끝은 바닥이지 측정이 아니다.** 320×180 아래에서 "접촉 없음" 이
> 빠르게 늘어난다(6 → 22 → 26 → 31 → 34 → 44 → 50 단). 덩어리 검출이
> 접촉을 놓치는 것이 섞여 있으므로, 160×90 미만의 값은 **분해 실패의 상한**
...
|---|---|---|---|---:|
| 9DTact | Fz | **80×45** | 32×18 ~ 640×360 | 9 |
...
| DIGIT_Marker | 전단 | **160×90** | 80×45 ~ 1280×720 | 9 |
...
| medium | 80×45 | 640×360 | 48×27 |
| hard | 426×240 | 640×360 | 32×18 |

<sub>유닛 9 개 — 두께 ρ -0.534 (p 0.139, q 0.857) · 경도 ρ +0.187 (p 0.630, q 0.986)</sub>

**9DTact — 전단 (Fx·Fy 평균)**

| 경도 | 1 mm | 2 mm | 3 mm |
|---|---|---|---|
| soft | 48×27 | 80×45 | 160×90 |
| medium | 48×27 | 48×27 | 32×18 |
| hard | 48×27 | 80×45 | 48×27 |

<sub>유닛 9 개 — 두께 ρ +0.116 (p 0.766, q 0.986) · 경도 ρ -0.290 (p 0.449, q 0.857)</sub>

**DIGIT — Fz (수직력)**

| 경도 | 1 mm | 2 mm | 3 mm |
|---|---|---|---|
| soft | 48×27 | 320×180 | 48×27 |
| medium | 80×45 | 80×45 | 32×18 |
| hard | 80×45 | 48×27 | 80×45 |

<sub>유닛 9 개 — 두께 ρ -0.308 (p 0.419, q 0.857) · 경도 ρ +0.056 (p 0.886, q 0.986)</sub>
...
<sub>유닛 9 개 — 두께 ρ -0.406 (p 0.278, q 0.857) · 경도 ρ +0.145 (p 0.710, q 0.986)</sub>
...
<sub>유닛 9 개 — 두께 ρ -0.348 (p 0.359, q 0.857) · 경도 ρ +0.000 (p 1.000, q 1.000)</sub>
...
| medium | 80×45 | 80×45 | 640×360 |
| hard | 320×180 | 1280×720 | 80×45 |

<sub>유닛 9 개 — 두께 ρ +0.055 (p 0.888, q 0.986) · 경도 ρ +0.277 (p 0.471, q 0.857)</sub>
...
<sub>유닛 9 개 — 두께 ρ +0.438 (p 0.238, q 0.857) · 경도 ρ -0.438 (p 0.238, q 0.857)</sub>
...
<sub>유닛 9 개 — 두께 ρ +0.600 (p 0.088, q 0.857) · 경도 ρ -0.164 (p 0.674, q 0.986)</sub>
...
<sub>유닛 9 개 — 두께 ρ +0.282 (p 0.463, q 0.857) · 경도 ρ -0.438 (p 0.238, q 0.857)</sub>
...
<sub>유닛 9 개 — 두께 ρ -0.059 (p 0.880, q 0.986) · 경도 ρ +0.000 (p 1.000, q 1.000)</sub>
