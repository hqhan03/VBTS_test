...
![Figure 2b — 같은 배치로 그린 합력 오차](../paper/figures/fig2b_resultant_force.png)

**Figure 2 와 같은 축·같은 규칙**이고, 두 열(Fz·전단) 대신 둘을 합친 힘 벡터의 오차 하나다. 바로 위와 견주면 된다. y 는 **하한**이다 — 정확한 E‖ΔF‖ 는 프레임별 예측이 있어야 나오는데 스윕이 축별 MAE 만 내보내므로, L2 노름의 볼록성으로 √(MAEx²+MAEy²+MAEz²) ≤ E‖ΔF‖ 를 쓴다. **합력에서도 굵은 선 셋이 가는 선들의 산포 안에 있다** — 축을 합쳐도 겔 설계가 곡선을 가르지 않는다.

### Figure 4 — 형상 복원

![Figure 4 — 형상 복원](../paper/figures/fig4_shape.png)

위 세 칸은 같은 프레임을 320·80·16 px 로 복원한 높이맵이고, 흰 막대가 참 지름 4 mm 다. (a) 단면, (b) **모양과 깊이 배율을 갈라 보인다** — 상관이 0.99 여도 깊이가 참값의 0.61 배에서 멈추는 유닛이 있다, (c) 깊이 오차 대 해상도, (d) **되찾은 가로 크기** 대 해상도. **(d) 만 깊이 참조형이다** — 가로 크기는 그쪽에만 측정돼 있다(광도 스테레오 파이프라인은 화소별 깊이만 채점한다). 접촉 중심은 원해상도에서 한 번 찾아 세 해상도가 함께 쓴다.

### Figure 4b — 겔 설계로 가른 형상 복원 오차

![Figure 4b — 겔 설계로 가른 형상 복원 오차](../paper/figures/fig4b_shape_by_gel.png)

**Figure 2b 와 같은 배치**를 형상에 건다 — 행이 원리 × 압자, 왼쪽은 두께로 오른쪽은 경도로 같은 자료를 두 번 나눈다. **y 축은 원리끼리만 공유한다** — 두 파이프라인은 서로 다른 깊이 구간을 평가하므로 절대 오차를 세로로 견주면 안 된다. 행 묶음 안에서 해상도에 따른 모양만 읽을 것.

<sub>만드는 것은 `src/scripts/paper_fig1.py` ~ `paper_fig4.py`. Figure 2·4 는 복제 선택 규칙으로 고른 시편 9 개를 쓴다 — 이 문서와 같은 집합이다. **Figure 3(겔 효과와 반복성)은 싣지 않는다** — 같은 조건의 r1·r2 를 선으로 잇는 그림인데 이 문서는 조건당 시편이 하나라 이을 상대가 없다. 그 그림과 그것이 재는 복제 산포는 `results.md` 에 있다.</sub>
...
| DIGIT | `hard_1mm_r2` · `hard_2mm_r2` · `hard_3mm_r1` · `medium_1mm_r1` · `medium_2mm_r1` · `medium_3mm_r1` · `soft_1mm_r2` · `soft_2mm_r1` · `soft_3mm_r1` |
...
움직이는 곳이 있는데(9DTact 의 Fz 가 R ≈ 54 에서 217 px/mm² 로), 유닛이
...
![천장 대 두께, 경도별 — 세 원리. 점이 유닛 하나하나이고 선이 그것을 잇는다. 검은 테두리에 `!` 가 붙은 점이 빛 누출 의심 유닛이고, 채움은 자기 경도의 색 그대로다. **세로 축은 원리마다 다르다** — 9DTact 가 63 N 까지 가는데 DIGIT 계열은 6 ~ 23 N 이라 축을 묶으면 DIGIT 과 Marker 의 기울기가 눌려 보이지 않는다.](single/extra/figures/B_ceiling_vs_thickness.png)

*천장 대 두께, 경도별 — 세 원리. 점이 유닛 하나하나이고 선이 그것을 잇는다. 검은 테두리에 `!` 가 붙은 점이 빛 누출 의심 유닛이고, 채움은 자기 경도의 색 그대로다. **세로 축은 원리마다 다르다** — 9DTact 가 63 N 까지 가는데 DIGIT 계열은 6 ~ 23 N 이라 축을 묶으면 DIGIT 과 Marker 의 기울기가 눌려 보이지 않는다.*
...
칸마다 점이 하나이므로 선이 곧 그 시편의 값이다. **이 기울기가 겔 설계의
것인지 제작물 간 변이의 것인지는 이 집합으로 가를 수 없다** — 같은 그림을
복제 둘로 그린 것이 `results.md` 에 있고, 거기서는 한 칸 안의 두 시편이
24.6 과 30.2 N 으로 벌어지기도 한다.
...
![3×3 표와 같은 숫자를 그림으로. 위는 분해된 가장 좁은 간격 대 두께(점이 유닛 하나하나다), 아래는 유닛마다의 분해된 깊이 범위.](single/extra/figures/G_spatial_resolution.png)

*3×3 표와 같은 숫자를 그림으로. 위는 분해된 가장 좁은 간격 대 두께(점이 유닛 하나하나다), 아래는 유닛마다의 분해된 깊이 범위.*
...
![두 점 판정 대 입력 해상도 (9DTact, pair100, 중심 간격 2.0 mm). 왼쪽은 분해된 사다리 단의 비율, 가운데는 골 깊이와 Rayleigh 문턱, 오른쪽은 자국 밝기와 판정 바닥.](single/extra/figures/H_resolution_sweep.png)

*두 점 판정 대 입력 해상도 (9DTact, pair100, 중심 간격 2.0 mm). 왼쪽은 분해된 사다리 단의 비율, 가운데는 골 깊이와 Rayleigh 문턱, 오른쪽은 자국 밝기와 판정 바닥.*

<sub>그림: `single/extra/figures/H_resolution_sweep.png` · 자료: `single/extra/data/H_resolution_sweep.csv`</sub>

**공간 분해능은 힘·형상보다 훨씬 촘촘한 화소를 쓴다.** 분해 비율이
R ≈ 387 px/mm² (426 px 폭) 에서 최대(70 %)이고 그 아래로 무너진다.
골 깊이가 Rayleigh 를 넘는 것은 R ≈ 13.6 까지이고 R ≈ 4.9 에서 0.20 으로
내려간다. 같은 9DTact 카메라에서 힘은 R ≈ 5~17, 형상은 R ≈ 4.9~5.5 에서 포화하는데,
두 점을 가르는 데는 R ≈ 387 이 든다 — **같은 카메라에서 과제가 요구하는 화소 밀도가 스무 배 넘게 벌어진다.**

**전해상도가 여기서도 손해다.** R ≈ 7862 (1920 px 폭) 의 분해 비율이 44 % 로 R ≈ 387 의 70 % 보다 낮다. 골 자체는 얕아지지 않는데(0.384 대 0.472)
**픽셀당 잡음이 5 배**여서(0.109 대 0.023) "골이 자기 잡음의 3 배" 관문이 걸린다. 자국 밝기도 2.15 로 판정 바닥 2.5 아래다.
...
위의 그림은 전부 **축별** MAE 다. 그런데 센서를 쓰는 쪽이 아는 것은 축이
아니라 **힘 벡터 하나**다 — "이 손가락이 힘을 몇 N 틀리게 읽는가" 는
‖F_pred − F_true‖ 의 크기다.

> **이 값은 지금 자료로 정확히 낼 수 없다.** 프레임마다 세 오차를 함께 봐야
> 하는데 스윕은 축별 MAE 만 내보내고 프레임별 예측을 버린다(가중치도 디스크에
> 없다). 그래서 **엄밀한 상·하한**을 친다. L2 노름이 볼록하므로 옌센
> 부등식으로 √(MAEx² + MAEy² + MAEz²) ≤ E‖ΔF‖ ≤ MAEx + MAEy + MAEz 이다.
> **띠는 불확실성이 아니라 부등식의 폭**이고, 인용할 값은 하한이다 — 세 축이
> 프레임마다 함께 틀릴 때 하한이 참값과 같아지는데, 압입이 세 축을 동시에
> 흔드는 이 실험이 그 경우에 가깝다.

![합력 오차 대 화소 밀도. (a) 굵은 선이 하한, 가는 선과 띠가 상한까지의 폭이다. (b) 제곱합에서 Fz 가 차지하는 몫 — 세 축이 같다면 33 % 다.](single/extra/figures/I_resultant_force.png)

*합력 오차 대 화소 밀도. (a) 굵은 선이 하한, 가는 선과 띠가 상한까지의 폭이다. (b) 제곱합에서 Fz 가 차지하는 몫 — 세 축이 같다면 33 % 다.*

<sub>그림: `single/extra/figures/I_resultant_force.png` · 자료: `single/extra/data/I_resultant_force.csv`</sub>

| 원리 | 합력 오차 바닥 (N) | 그때 폭 | 포화 R | Fz 의 몫 |
|---|---:|---:|---:|---:|
| 9DTact | **0.0578** | 640 px | 13.7 | 63 % |
| DIGIT | **0.0726** | 80 px | 18.3 | 66 % |
| DIGIT_Marker | **0.0949** | 320 px | 361 | 79 % |

**합력 오차의 바닥은 원리 순서를 그대로 따른다** — 9DTact 0.058, DIGIT 0.073, DIGIT_Marker 0.095 N. 세 곡선 모두
R ≈ 10 아래에서 가파르게 나빠지고 그 위로는 평평하다.

**합력은 사실상 Fz 다.** 제곱합에서 Fz 의 몫이 9DTact 63 %, DIGIT 66 %, DIGIT_Marker 79 % 로 셋 다 등분선
(33 %)보다 한참 위이고, **그 비중이 해상도를 따라 움직이지 않는다.** 즉
합력 곡선의 모양은 Fz 곡선의 모양이고, 전단은 크기를 조금 올릴 뿐이다.
마커에서 이 쏠림이 가장 심한 것은 마커가 전단을 돕고(H9) Fz 는 돕지
않기 때문이다.

> **포화 R 열은 조심해서 읽을 것.** 합력 곡선의 평평한 구간은 폭이 넓고
> 얕아 110 % 규칙이 유닛 수에 민감하다 — 같은 계산을 전체 18 유닛으로 하면
> 값이 한두 단 움직인다. 움직이지 않는 것은 **바닥의 크기와 원리 순서**다.

**실무적으로**: 세 원리 모두 합력 오차 0.06 ~ 0.09 N
이고 측정 범위가 0 ~ 2.1 N 이므로 **범위의 3 ~ 5 %** 다. 그 값을
더 낮추려면 해상도가 아니라 Fz 를 고쳐야 한다.

<sub>만드는 것은 `scripts/resultant_force.py` · 자료 `extra/data/I_resultant_force.csv`, 요약 `..._summary.csv`. 정확한 값을
원하면 `force_vs_resolution.py` 가 이제 내보내는 `res_mae`(벡터 오차)와
`mag_mae`(크기 오차)를 쓰면 되지만, 전 사다리 재학습 약 21 GPU·시간이
든다.</sub>
...
칸마다 시편이 하나이므로 칸에 값이 하나씩 들어간다. **검정 단위는 유닛**이다.
...
|---|---|---:|---|---|---:|
| 9DTact | Fz | **17.3** | 1.66 ~ 880 | 80×45 | 9 |
| 9DTact | 전단 | **5.89** | 1.88 ~ 44.7 | 48×27 | 9 |
| DIGIT | Fz | **13.3** | 2.19 ~ 293 | 80×45 | 9 |
| DIGIT | 전단 | **4.92** | 2.38 ~ 590 | 48×27 | 9 |
| DIGIT_Marker | Fz | **23.0** | 2.82 ~ 455 | 80×45 | 9 |
| DIGIT_Marker | 전단 | **71.7** | 16.5 ~ 5,898 | 160×90 | 9 |
...
|---|---|---|---|
| soft | 17.3 | 218 | 4.03 |
| medium | 15.4 | 870 | 4.23 |
| hard | 464.0 | 880 | 1.66 |

<sub>유닛 9 개 — 두께 ρ -0.527 (p 0.145, q 0.724) · 경도 ρ +0.211 (p 0.586, q 0.893)</sub>
...
|---|---|---|---|
| soft | 6.23 | 13.6 | 44.7 |
| medium | 5.53 | 4.89 | 1.88 |
| hard | 5.89 | 13.8 | 3.73 |

<sub>유닛 9 개 — 두께 ρ -0.158 (p 0.685, q 0.893) · 경도 ρ -0.369 (p 0.329, q 0.893)</sub>
...
|---|---|---|---|
| soft | 7.49 | 293 | 5.36 |
| medium | 21.1 | 17.4 | 2.19 |
| hard | 23.0 | 6.74 | 13.3 |

<sub>유닛 9 개 — 두께 ρ -0.580 (p 0.102, q 0.679) · 경도 ρ +0.053 (p 0.893, q 0.893)</sub>

**DIGIT — 전단 (Fx·Fy 평균)**

| 경도 | 1 mm | 2 mm | 3 mm |
|---|---|---|---|
| soft | 590.0 | 6.60 | 2.38 |
| medium | 3.37 | 6.25 | 4.92 |
| hard | 3.68 | 6.74 | 4.77 |

<sub>유닛 9 개 — 두께 ρ -0.211 (p 0.586, q 0.893) · 경도 ρ -0.105 (p 0.787, q 0.893)</sub>
...
|---|---|---|---|
| soft | 27.0 | 22.6 | 287 |
| medium | 89.6 | 22.7 | 2.82 |
| hard | 455.0 | 23.0 | 16.5 |
...
| 경도 | 1 mm | 2 mm | 3 mm |
|---|---|---|---|
| soft | 108.0 | 22.6 | 71.7 |
| medium | 22.4 | 22.7 | 1,129 |
| hard | 455.0 | 5,898 | 16.5 |

<sub>유닛 9 개 — 두께 ρ -0.053 (p 0.893, q 0.893) · 경도 ρ +0.158 (p 0.685, q 0.893)</sub>
...
**이 문서는 조건당 시편이 하나이므로 복제 산포를 재지 않는다.** 유닛별
포화 해상도가 넓게 흩어진다는 것은 위 표의 범위 열에서 보이지만, 그 산포
중 얼마가 제작물 간 변이이고 얼마가 설계 인자인지는 이 집합으로 가를 수
없다. 복제 쌍으로 잰 값은 `results.md` 에 있다.
...
압자는 ⌀4 mm 원기둥과 한 변 4 mm 정육면체다. **형상 센서가 되찾아야 할 것은
깊이와 가로 크기 둘**인데, 이 절의 MAE 는 전부 깊이다. 9DTact 는 반깊이 윤곽의
지름도 재 두었으므로(`cyl4_raw_size`, `cube4_raw_size`) 함께 읽는다 — Figure 4(d).

| 화소 밀도 R | 원기둥 ⌀4 | 정육면체 4 |
|---|---:|---:|
| 7 830 (1920 px) | 4.13 | 4.82 |
...
**가로 크기는 깊이처럼 포화하지 않는다.** 깊이 MAE 는 R ≈ 54 위로 평평한데,
정육면체의 지름 오차는 **전해상도에서 가장 크고**(+0.82 mm, 20 %) 해상도를
내릴수록 줄어 R ≈ 2 에서 +0.08 mm 가 된다. 원기둥은 R ≈ 54 에서 가장
정확하고(+0.10 mm) R ≈ 4.9 에서 −0.43 mm 로 가장 작게 나온다.

> **이것을 "낮은 해상도가 더 낫다" 로 읽지 말 것.** 정육면체의 +0.82 mm 는
> 평면 압자의 반깊이 윤곽이 참 모서리 **바깥**에 서기 때문이고, 해상도를 내리면
> 흐려짐이 그 윤곽을 안쪽으로 당겨 우연히 참값과 만난다. 두 오차가 상쇄된
> 것이지 측정이 좋아진 것이 아니다. R ≈ 0.5 (16 px) 아래에서는 반깊이 윤곽이
> 화면 전체가 되므로 값이 아니라 검출 실패이고, 그림과 표에서 뺐다.

<sub>자료: `../paper/figures/fig4_lateral_size.csv`</sub>

![9DTact — 형상 복원 오차 대 화소 밀도. 둘 다 R ≈ 54 px/mm² 에서 최소.](single/1_9DTact/figures/shape_mae_summary.png)
...
에서 두 압자의 차이 중앙이 9DTact +0.48 mm, DIGIT +0.85 mm 이고, 방향이
같은 것이 각각 **90 %, 81 %** 다. 평면과 모서리를 가진 압자의 반깊이 윤곽이
...
|---|---:|---:|---:|---:|
| 1 mm | +0.08 | +0.34 | −0.36 | +0.15 |
| 2 mm | +0.27 | +0.54 | −1.02 | −0.08 |
| 3 mm | +0.72 | +1.24 | −0.31 | +0.63 |
...
> 선이 그것이고, 값이 아니라 검출 실패다.
>
> 2026-09-14 에 면적을 **윤곽 다각형이 아니라 마스크 화소수**로 재도록
> 고쳤다. `contourArea` 는 경계를 반 화소 안쪽으로 잡아 큰 자국에서는
> 오차가 0.4 % 지만 작은 자국에서는 화소수의 40 % 까지 내려가고, 그래서
> 저해상도 점들이 통째로 잘려 있었다 — DIGIT 16×9 가 16 행에서 **80 행**
> 으로, 9DTact 8×5 가 1 유닛에서 5 유닛으로 늘었다. 전해상도 값은 0.5 %
> 움직였다.
...
**두 파이프라인 다 예전에 적은 것보다 훨씬 낮은 해상도를 견딘다.** DIGIT 의
`cyl4` 는 R ≈ 10 389 (1920×1080) 의 0.048 mm 에서 R ≈ 2.9 (32×18) 의
0.055 mm 까지, 여섯 단을 내려오는 동안 15 % 밖에 나빠지지 않는다. 9DTact 는
R ≈ 5.5 (48×27) 까지 0.062 ~ 0.072 mm 로 평평하다. **두 포화 해상도가 1.9 배
안에 든다.**

> **2026-09-14 에 두 파이프라인에서 같은 종류의 결함을 찾아 고쳤다.** 둘 다
> 픽셀 단위로 고정된 필터가 입력이 줄면서 자국보다 커진 것이다.
>
> - **DIGIT**: 접촉 마스크의 3×3 열림. 16×9 에서 자국 18 px 이 **0 px** 이
>   되어 깊이가 통째로 0 이 됐다. 8×5 프레임의 80 %, 16×9 의 74 %, 32×18 의
>   52 % 가 정확히 0 을 예측했다 — 복원 오차가 아니라 검출 실패였다.
> - **9DTact**: 7×7 가우시안 **셋**(차영상 하나, 깊이맵 둘). 48×27 의 MAE 가
>   0.155 → 0.072 mm 로 바뀌었다.
>
> 둘 다 커널을 축소 배율에 비례시켰고, **1920×1080 의 값은 바뀌지 않았다.**
> 그 전에는 둘 다 80×45 에서 포화하는 것으로 보였는데, 서로 다른 두 필터의
> 붕괴가 우연히 비슷한 자리에서 일어났기 때문이다.
> `docs/shape_reconstruction.md` §5 와 §6.5a.
...
| 원리 | 지표 | 중앙 포화 해상도 R (px/mm²) | 유닛별 범위 | 해당 픽셀 폭 | n |
|---|---|---:|---|---|---:|
| 9DTact | cyl4 | **5.53** | 1.66 ~ 1,549 | 48×27 | 9 |
| 9DTact | cube4 | **4.91** | 2.20 ~ 7,830 | 48×27 | 9 |
| DIGIT | cyl4 | **2.19** | 0.19 ~ 1,111 | 32×18 | 9 |
| DIGIT | cube4 | **83.2** | 0.21 ~ 1,199 | 160×90 | 9 |
...
|---|---|---|---|
| soft | 69.3 | 4.91 | 4.03 |
| medium | 5.53 | 1,549 | 752 |
| hard | 16.4 | 2.20 | 1.66 |

<sub>유닛 9 개 — 두께 ρ -0.316 (p 0.407, q 0.893) · 경도 ρ -0.264 (p 0.493, q 0.893)</sub>

**9DTact — 정육면체 4 mm**

| 경도 | 1 mm | 2 mm | 3 mm |
|---|---|---|---|
| soft | 69.3 | 4.91 | 1,275 |
| medium | 2.46 | 7,830 | 188 |
| hard | 2.62 | 2.20 | 3.73 |

<sub>유닛 9 개 — 두께 ρ +0.422 (p 0.258, q 0.861) · 경도 ρ -0.580 (p 0.102, q 0.679)</sub>
...
|---|---|---|---|
| soft | 83.2 | 2.93 | 0.60 |
| medium | 0.84 | 1,111 | 2.19 |
| hard | 0.92 | 0.19 | 13.3 |

<sub>유닛 9 개 — 두께 ρ -0.053 (p 0.893, q 0.893) · 경도 ρ -0.211 (p 0.586, q 0.893)</sub>

**DIGIT — 정육면체 4 mm**

| 경도 | 1 mm | 2 mm | 3 mm |
|---|---|---|---|
| soft | 83.2 | 1,173 | 2.38 |
| medium | 0.21 | 6.25 | 875 |
| hard | 0.23 | 1,199 | 212 |

<sub>유닛 9 개 — 두께 ρ +0.422 (p 0.258, q 0.861) · 경도 ρ +0.053 (p 0.893, q 0.893)</sub>
...
보정의 근거는 5 절에 한 번만 적었다. **어느 칸도 q < 0.05 가
아니다.**
