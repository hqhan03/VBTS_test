...
> **깃헙에는 측정 데이터가 없다.** 이 기계에는 92,555 개 파일이 있지만 깃이 추적하는 것은
> 60 개 — 분석이 낸 표(`analysis/`), 문서(`*.md`), Pass B 의 `CANONICAL.yaml`, 그리고
> 로봇이 매 런마다 읽는 `rig/frame_transform/` 뿐이다. 아래 크기는 **이 기계 기준**이다.
> 런 자체(사진·`state.json`·`ladder.csv`·`stream/`)와 `_discarded/` 는 여기 있고
> 깃헙에는 없다. 규칙은 저장소 루트의 `.gitignore` 맨 아래.
