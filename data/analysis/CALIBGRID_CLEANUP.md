...
| 내용 | 이름 |
|---|---|
| 분석이 쓰는 격자 (정렬됨, 0.15 N 미만 프레임이 가장 적음) | `<유닛>/calibgrid_ball4/` |
| 등록부의 천장이 나온 램프 | `<유닛>/characterize/` |
| 그 외 | `_discarded/<원리>/20260910_passA_calibgrid/` |

**유닛당 폴더는 정확히 하나다.** 격자와 천장이 서로 다른 런에서 나온 12 유닛은 램프를
격자 폴더로 옮겨 합쳤고, 그 램프 옆에 `characterize/PROVENANCE.yaml` 을 두어 어느 런에서
왔는지와 **그 런 자신의 영점 표면**을 적었다 — `steps.csv` 의 `depth_mm` 는 그 영점 기준이지
같은 폴더의 `state.json`(격자 런) 기준이 아니다.
...
## 뒤늦게 고친 것 — 버린 폴더가 `data/` 밖에 있었다 (2026-09-11)

이 정리가 만든 `_discarded` 34 런(1.8 GB)이 저장소 **루트**에 붙어 있었다. 경로를
`data/` 기준이 아니라 저장소 기준으로 잡은 탓이다. 코드는 전부 `data/_discarded` 만
보므로 이것들은 어느 분석에도 걸리지 않았지만, 깃헙에는 그대로 올라가 있었다.
`data/_discarded/<원리>/<데이터셋>/` 로 합쳤다 — 이름 충돌 없음.
