...
def ensure_clear(ip: str, a, sensor_entry: dict) -> bool:
    ...
    if not run([PY, str(ROOT / "src" / "scripts" / "move_probe.py"), "--test-up",
...
def main() -> int:
    ...
    try:
        _cp = yaml.safe_load(open(ROOT / "src" / "config" / "sensor_registry.yaml"))["capture_policy"]
    ...
    R = [PY, str(ROOT / "src" / "scripts" / "run_indentation.py")]
    ...
    if a.dataset:
        ...
        R += ["--dataset", a.dataset]
    M = [PY, str(ROOT / "src" / "scripts" / "move_probe.py")]
    surface = None
    run_dir = None
    t0 = time.time()
    ...
    for step in order:
        t_step = time.time()
        if step == "enable":
            if not run([PY, str(ROOT / "src" / "scripts" / "enable_protocol_motion.py"),
            ...
            if not _recentre(ip, a, M):
                return 1
            reg2 = yaml.safe_load(open(ROOT / "src" / "config" / "sensor_registry.yaml"))
...
def park_whatever_happens(rc: int) -> int:
    ...
    if rc == 0 and "--no-exit-park" in argv:
        ...
        return rc
    try:
        ip = yaml.safe_load(open(ROOT / "src" / "config" / "robot_config.yaml"))["robot"]["ip"]
        if "--ip" in argv:
        ...
        r = subprocess.run([PY, str(ROOT / "src" / "scripts" / "move_probe.py"),
                            "--test-up", f"{want - h:.3f}",
