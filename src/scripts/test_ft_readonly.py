...
no robot access.
    ...
    /usr/bin/python3 src/scripts/test_ft_readonly.py
    /usr/bin/python3 src/scripts/test_ft_readonly.py --seconds 5
"""
...
def main() -> int:
    ...
    if bad:
        ...
        print("correct daq.channels in src/config/ft_config.yaml before believing any force.")
    elif not ft.channel_mapping_confirmed:
        print("RESULT: all six channels healthy, but the SG->ai mapping is not yet")
        print("confirmed against a known load. Run src/scripts/test_weight_validation.py.")
    else:
