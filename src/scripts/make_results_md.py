...
# ---------------------------------------------------------------- 4 --
    ...
    w("**DIGIT 은 9DTact 보다 훨씬 낮은 해상도를 견딘다.** `cyl4` 가 R ≈ 10 389")
    w("(1920×1080) 의 0.048 mm 에서 R ≈ 2.9 (32×18) 의 0.055 mm 로, 여섯 단을")
    w("내려오는 동안 15 % 밖에 나빠지지 않는다. 나빠지는 것은 R ≈ 0.7 (0.081) 과")
    w("R ≈ 0.18 (0.129) 뿐이다. 9DTact 가 R ≈ 54 에서 이미 바닥을 치고 그 아래로")
    w("무너지는 것과 대조된다 — **열여덟 배** 차이다.")
    w()
    w("> **2026-09-14 에 고친 결함이다.** 그 전에는 DIGIT 도 R ≈ 18 (80×45) 에서")
    w("> 포화하는 것으로 보였다. `predict_depth` 의 접촉 마스크가 3×3 열림을 픽셀")
    w("> 단위로 고정해 두어 저해상도에서 커널이 자국보다 커졌고, 16×9 에서 자국")
    w("> 18 px 이 **0 px** 이 되어 깊이가 통째로 0 이 됐다. 8×5 에서 프레임의 80 %,")
    w("> 16×9 에서 74 %, 32×18 에서 52 % 가 정확히 0 을 예측했다 — 복원 오차가")
    w("> 아니라 검출 실패였다. 커널을 폭에 비례시켜 다시 냈고, **1920×1080 의 값은")
    w("> 한 자리도 바뀌지 않았다.** `docs/shape_reconstruction.md` §5.")
    w()
    w("> 왜 광도 스테레오가 더 잘 버티는가 — 조회표는 화소마다 독립적으로 깊이를")
    w("> 읽지만 광도 스테레오는 **기울기를 적분**하므로 화소 몇 개만 있어도 모양이")
    w("> 남는다. 다만 이것은 사후 설명이고 검정한 예측이 아니다.")
    w()
