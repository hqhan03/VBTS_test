...
def main():
    ...
    w("## 0. 한눈에")
    w()
    w("> **제외된 유닛.** `9DTact_hard_1mm_r2` 와 `9DTact_medium_1mm_r2` 는 운전자가")
    ...
    w("> +24 %, +25 % 부풀었다. 겔이 아니라 **바디**를 재고 있으므로 이 문서의 모든")
    w("> 그림·표·csv 에서 뺐다 — 9DTact 는 15 유닛이다. 3×3 표에서 `e` 가 붙은 칸은")
    w("> 그 때문에 복제가 하나뿐인 칸이다. 등록부의 `suspect_hardware` 가 근거다.")
    w()
