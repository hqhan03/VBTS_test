...
# ---------------------------------------------------------------- 4 --
    ...
    w("R ≈ 385 px/mm² (426×240) 에서 최대(71 %)이고 R ≈ 217 아래로 무너진다.")
    w("골 깊이가 Rayleigh 를 넘는 것은 R ≈ 13.6 까지이고 R ≈ 4.9 에서 0.20 으로")
    w("내려간다. 같은 9DTact 카메라에서 힘은 R ≈ 5~17, 형상은 R ≈ 53~59 에서")
    w("포화한다 — **세 과제가 요구하는 화소 밀도가 70 배 벌어진다.**")
    w()
    w("**전해상도가 여기서도 손해다.** R ≈ 7830 (1920×1080) 의 분해 비율이 39 % 로")
    w("R ≈ 385 의 71 % 보다 낮다. 골 자체는 얕아지지 않는데(0.384 대 0.464)")
    w("**픽셀당 잡음이 네 배**여서(0.108 대 0.026) \"골이 자기 잡음의 3 배\" 관문이")
    w("걸린다. 자국 밝기도 2.15 로 판정 바닥 2.5 아래다.")
    w()
