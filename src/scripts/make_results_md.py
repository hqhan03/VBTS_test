...
# ---------------------------------------------------------------- 4 --
    ...
    w("> **정정 진행 중 (2026-09-13).** 아래 \"실효 배치 64\" 가 **세 해상도에서 사실이")
    w("> 아니었다** — 640×360 에서 44, 854×480 에서 72, 1280×720 에서 66 이었다.")
    w("> `accum = round(batch / micro)` 의 반올림 때문이고, 나머지 아홉 단은 정확히")
    w("> 64 다. 미세배치가 배치를 나누도록 고쳐 **그 세 단만** 다시 학습하는 중이다.")
    w("> 끝나면 이 경고를 떼고 숫자를 갱신한다.")
    w()
