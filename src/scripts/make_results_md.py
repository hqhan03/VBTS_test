...
def knee_block(family, metrics, what, tail=True):
    ...
    if not tail:
        ...
        w("보정의 근거와 복제 재현성은 5 절에 한 번만 적었다. **어느 칸도 q < 0.05 가")
        w("아니다.**")
        w()
    if not tail:
        return
...
def main():
    ...
    fig("extra/figures/B_ceiling_vs_thickness.png",
        ...
        "세로 막대는 두 복제의 폭. 검은 테두리에 `!` 가 붙은 점이 빛 누출 의심 "
    ...
    for pr in ("9DTact", "DIGIT", "DIGIT_Marker"):
        for probe in ("ball8", "ball4"):
            ...
            if (RES / f).exists():
                ...
                break
    w("### 깊이 기준과 힘 기준이 서로 다른 답을 준다")
    w()
    w("2 절과 같은 방식으로 **군 사이 차이 ÷ 군 안의 사분위 폭**을 잰다. 1 보다 크면")
    w("그 변수가 유닛 산포 위로 올라온다는 뜻이다.")
    w()
    w("| 원리 | 잰 것 | 나눈 것 | 깊이 기준 비 | **힘 기준 비** |")
    w("|---|---|---|---:|---:|")
    import itertools
    MN2 = {"diameter_px": "자국 지름", "level": "밝기 변화"}
    KN2 = {"hardness": "경도", "thickness_mm": "두께"}
    for pr, pb in (("9DTact", "ball8"), ("DIGIT", "ball8"),
                   ("DIGIT_Marker", "ball4")):
        fa = RES / FOLD[pr] / "data" / f"optical_group_spread_{pb}.csv"
        fb = RES / FOLD[pr] / "data" / f"force_group_spread_{pb}.csv"
        if not (fa.exists() and fb.exists()):
            continue
        A, B = pd.read_csv(fa), pd.read_csv(fb)
        for meas, key in itertools.product(("diameter_px", "level"),
                                           ("hardness", "thickness_mm")):
            a = A[(A.measure == meas) & (A.group_by == key)]
            b = B[(B.measure == meas) & (B.group_by == key)]
            if not len(a) or not len(b):
                continue
            bv = b.ratio.iloc[0]
            w(f"| {pr} | {MN2[meas]} | {KN2[key]} | {a.ratio.iloc[0]:.2f} | "
              f"{'**' if bv >= 1 else ''}{bv:.2f}{'**' if bv >= 1 else ''} |")
    w()
    w("<sub>자료: `<원리>/data/force_group_spread_<프로브>.csv` · 곡선 "
      "`force_by_group_<프로브>.csv`</sub>")
    w()
    w("**9DTact 에서 정확히 뒤집힌다.** 자국 지름을 깊이에 대해 보면 두께가 갈리고")
    w("(비 10.35) 경도는 묻히는데(0.41), 힘에 대해 보면 **경도가 갈리고(3.85) 두께가")
    w("사라진다(0.38).** 같은 자국, 같은 램프, 바뀐 것은 x 축뿐이다.")
    w()
    ...
    w()
    w("> **DIGIT 계열에서는 뒤집히지 않는다.** 두께가 두 기준 모두에서 갈리고(2.0 ~ 2.9)")
    w("> 경도는 힘 기준에서 겨우 1.3 까지만 올라온다. 9DTact 는 경도를 40 Shore 점")
...
# ---------------------------------------------------------------- 4 --
    ...
    w("### 유닛별 — 경도 × 두께 × 복제")
    w()
    w("5 절의 힘 그림과 같은 배치다. 행이 경도, 열이 두께와 복제이고, 선 하나가 평가")
