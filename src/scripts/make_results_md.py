...
# ---------------------------------------------------------------- 4 --
    ...
    w("에서 두 압자의 차이 중앙이 9DTact +0.56 mm, DIGIT +0.85 mm 이고, 방향이")
    w("같은 것이 각각 **93 %, 81 %** 다. 평면과 모서리를 가진 압자의 반깊이 윤곽이")
    ...
    w("|---|---:|---:|---:|---:|")
    w("| 1 mm | +0.00 | +0.34 | −0.39 | +0.08 |")
    w("| 2 mm | +0.11 | +0.54 | −1.07 | −0.15 |")
    w("| 3 mm | +0.60 | +1.24 | −0.35 | +0.59 |")
    w()
    ...
    w()
    w("**DIGIT 은 원기둥을 일관되게 작게 되찾는다**(−0.35 ~ −1.07 mm). 9DTact 가")
    w("0 근처인 것과 대조되지만, 두 파이프라인은 서로 다른 깊이 구간을 평가하므로")
    w("이 차이로 우열을 매기지 않는다 — 방향이 다르다는 것만 기록한다.")
    w()
    w("> **가장 왼쪽 한두 점은 읽지 말 것.** R ≈ 0.2 (8×5) 에서는 반깊이 윤곽이")
    w("> 화면을 덮거나 아예 잡히지 않는다. 축 밖으로 솟는 선이 그것이고, 값이")
    w("> 아니라 검출 실패다.")
    w()
