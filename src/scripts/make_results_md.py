...
# ---------------------------------------------------------------- 4 --
    ...
    fig("extra/figures/H_resolution_sweep.png",
        ...
        "오른쪽은 자국 밝기와 판정 바닥.")
    w("**공간 분해능은 힘·형상보다 훨씬 많은 픽셀을 쓴다.** 분해 비율이 426×240 에서")
    w("최대(71 %)이고 320×180 아래로 무너진다. 골 깊이가 Rayleigh 를 넘는 것은")
    w("80×45 까지이고 48×27 에서 0.20 으로 내려간다. 힘은 48×27, 형상은 80×45 ~")
    w("160×90 에서 포화하므로 **같은 카메라에서 세 과제가 서로 다른 픽셀을 요구한다.**")
    w()
    w("**전해상도가 여기서도 손해다.** 1920×1080 의 분해 비율이 39 % 로 426×240 의")
    w("71 % 보다 낮다. 골 자체는 얕아지지 않는데(0.384 대 0.464) **픽셀당 잡음이")
    w("네 배**여서(0.108 대 0.026) \"골이 자기 잡음의 3 배\" 관문이 걸린다. 자국")
    w("밝기도 2.15 로 판정 바닥 2.5 아래다.")
    w()
    w("> **낮은 쪽 끝은 바닥이지 측정이 아니다.** 320×180 아래에서 \"접촉 없음\" 이")
    w("> 빠르게 늘어난다(6 → 22 → 26 → 31 → 34 → 44 → 50 단). 덩어리 검출이")
    w("> 접촉을 놓치는 것이 섞여 있으므로, 160×90 미만의 값은 **분해 실패의 상한**")
