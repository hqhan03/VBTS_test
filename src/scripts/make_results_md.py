...
def gel_block():
    ...
    w()
    w("**선은 다른 인자를 고정한 것**이다. 가로가 두께면 선 셋은 soft·medium·")
    w("hard 각각의 두께 곡선이고, 가로가 경도면 1·2·3 mm 각각의 경도 곡선이다.")
    w("아홉 유닛이 3 × 3 이라 칸마다 하나씩이므로 **선마다 점이 정확히 셋**이다.")
    w("선이 나란하면 두 인자가 더해지는 것이고, **엇갈리면 상호작용**이다.")
    w()
    for f, cap in [("extra/figures/J_gel_effect_force.png",
                    "힘 추정 — 겔 설계에 따른 Fz MAE 와 R². 점 하나가 유닛 "
                    "하나이고, 선은 다른 인자를 고정한 것이다."),
                   ("extra/figures/J_gel_effect_shape.png",
                    "형상 복원 — 겔 설계에 따른 원기둥 MAE 와 R². 점 하나가 "
                    "유닛 하나이고, 선은 다른 인자를 고정한 것이다.")]:
        if (RES / f).exists():
            fig(f, cap)
    ...
    if {"9DTact", "DIGIT"} <= set(both.columns):
        ...
        if len(opp):
            ...
            w("아니라 파이프라인의 성질이다.")
            w()
    w("**선이 나란한 것은 힘의 두께 쪽뿐이다.** 다른 인자를 고정하고 잰 기울기의")
    w("부호가 셋 다 같은지 세어 보면, Fz 는 세 원리 모두 **두께에 대해** 부호가")
    w("같다(MAE 는 9DTact +, DIGIT·마커 −). 그런데 **경도에 대해서는** 9DTact 와")
    w("마커만 같고 DIGIT 은 엇갈린다. 형상은 더 나쁘다 — 9DTact 는 네 조합 모두")
    w("엇갈리고, DIGIT 은 MAE 만 같고 R² 는 엇갈린다.")
    w()
    w("> **엇갈린다는 것은 ρ 하나로 요약하면 안 된다는 뜻이다.** 위 표의 Spearman")
    w("> 은 아홉 점을 한 줄에 세워 재는데, 선이 엇갈리면 그 한 줄이 존재하지")
    w("> 않는다. 그래서 이 절의 결론은 **\"두께 쪽에 신호가 몰린다\"** 까지이고,")
    w("> 칸별 크기를 인용할 근거는 되지 못한다.")
    w()
...
def main():
    ...
    for f, cap, note in [
        ...
        ("fig4b_shape_by_gel",
         ...
         "읽을 것."),
        ("fig_task_requirement",
         "Figure 5 — 한 카메라에서 두 과제가 요구하는 화소 밀도",
         "같은 9DTact 카메라에서 힘과 형상을 같은 x 축에 놓는다. 위 섬네일이 "
         "그 밀도에서 영상이 실제로 어떻게 보이는지다. **▲ 는 평탄 구간이 "
         "시작하는 곳**(유닛별 평탄점의 중앙), **✕ 는 평탄 오차의 두 배를 "
         "넘어 못 쓰게 되는 곳**이다. 띠는 아홉 유닛의 사분위 범위 — "
         "불확실성이 아니라 유닛 사이의 산포다. 힘 쪽에 Fz 가 아니라 **합력**을 "
         "그린 이유는 아래에 적었다."),
    ]:
...
# ---------------------------------------------------------------- 4 --
    ...
    # ---- 마커 전처리 ---------------------------------------------------
    _mr = "3_DIGIT_Marker/figures/marker_reps.png"
    if (RES / _mr).exists():
        w("### 마커 팔의 전처리 — 지운 것과 남긴 것")
        w()
        w("머리 결과의 마커 팔은 `inpaint`, 곧 마커 점을 **지운** 표현이다.")
        w("그런데 마커 기반 센서의 표준은 Marker Displacement Method 로 마커를")
        w("**추적**해 전단을 읽고, FEATS 는 GelSight Mini 원본 이미지를 그대로")
        w("넣는다. 우리는 그 신호를 지우고 학습했다(2026-09-14 운전자 지적).")
        w()
        w("그래서 마커를 남긴 `colour`·`grey` 로 선택 아홉 유닛을 축분리로 다시")
        w("학습했다. **같은 프레임, 같은 설정, 같은 학습 판이고 입력 표현만**")
        w("**다르다.**")
        w()
        fig(_mr, "DIGIT_Marker — 세 전처리를 나란히. 전단 · Fz · 합력.")
        w("**세 곡선이 거의 겹친다.** 전해상도 한 점에서 짝지으면 차이가 없다는")
        w("`force_estimation.md` §7.3b 의 결과가 해상도를 가로질러도 유지된다.")
        w()
        w("차이가 나는 곳은 **곡선의 오른쪽 끝**이다. 1920 px 가 80 px 보다 나은")
        w("유닛과 그 비(1920÷80):")
        w()
        w("| 전처리 | 나은 유닛 | p | 전단 비 |")
        w("|---|---:|---:|---:|")
        w("| `inpaint` (지움) | 7/9 | **0.039** | **0.85** |")
        w("| `colour` (남김) | 7/9 | 0.359 | 0.95 |")
        w("| `grey` (남김) | 6/9 | 0.301 | 0.94 |")
        w()
        w("**방향은 살아남고 세기는 살아남지 않는다.** 마커 없는 두 팔은")
        w("전해상도에서 34 ~ 36 % 나빠지는데(비 1.34 · 1.36) 마커 팔은 어느")
        w("전처리에서도 나빠지지 않는다. 다만 지우면 15 % 좋아지고 남기면")
        w("사실상 평평하다.")
        w()
        w("> 그러므로 정확한 진술은 **\"마커는 전단이 고해상도에서 나빠지는 것을")
        w("> 막는다\"** 이지 \"고해상도에서 좋아지게 한다\" 가 아니다. 뒤의 문장은")
        w("> `inpaint` 에서만 성립한다. `cross_principle.md` §3.5a.")
        w()
        w("<sub>만드는 것은 `scripts/marker_reps.py` · 자료 "
          "`3_DIGIT_Marker/data/marker_reps.csv`, 원자료 `reps_*.csv`</sub>")
        w()
    ...
    w()
    w("> **2026-09-14 에 다시 학습해 실측값을 얻었다.** 아래 그림의 굵은 선이")
    w("> 실측이고 점선이 하한, 띠가 부등식의 폭이다. 하한은 99 ~ 100 % 의 칸에서")
    w("> 성립했고 참값의 **0.86 ~ 0.90 배**였다 — 평탄점의 위치는 같게 짚었지만")
    w("> 크기를 10 % 쯤 낮게 잡고 있었다. 아래 단락은 그 하한을 쓰던 때의 설명이고,")
    w("> 부등식이 왜 필요했는지를 남겨 둔다.")
    w()
    ...
    if sf.exists():
        ...
        w()
    w("### 허용치를 조이면 답이 재현되지 않는다")
    w()
    w("**같은 아홉 시편을 같은 설정으로 두 번 학습했다.** 2026-09-12 판(A)과")
    w("합력을 얻으려고 돌린 2026-09-14 판(B)이고, 다른 것은 학습의 확률성뿐이다.")
    w("평탄 밀도를 세 허용치로 재면:")
    w()
    w("| 원리 | 판 | 20 % | 10 % | 5 % | 5/10 |")
    w("|---|---|---:|---:|---:|---:|")
    for pr, a_, b_ in [("9DTact", (13.6, 13.6, 13.8), (4.9, 13.6, 44.7)),
                       ("DIGIT", (3.4, 6.7, 18.7), (3.3, 4.9, 6.7)),
                       ("DIGIT_Marker", (8.1, 23.0, 27.0), (8.1, 22.4, 71.7))]:
        for lab, v in (("A", a_), ("B", b_)):
            w(f"| {pr} | {lab} | {v[0]:g} | {v[1]:g} | {v[2]:g} | "
              f"{v[2] / v[1]:.1f}× |")
    w()
    w("**20 % 와 10 % 는 사다리 한 단 안에서 맞는다. 5 % 는 맞지 않는다** — 밀도가")
    w("최대 세 배 움직이고, **어느 계열이 더 빨리 나빠지는지의 순서가 뒤바뀐다.**")
    w("A 판에서는 깊이 참조형이 1.0 배로 가장 단단하고 광도 스테레오가 2.8 배로")
    w("취약했는데, B 판에서는 정확히 반대다(3.3 배 대 1.4 배).")
    w()
    w("**10 % 에서도 Fz 혼자는 재현되지 않는다.** 위 표는 합력 기준이다. 같은 두")
    w("판을 지표별로 보면:")
    w()
    w("| 지표 (9DTact) | 판 A | 판 B | 배율 |")
    w("|---|---:|---:|---:|")
    w("| Fz | 17.3 | **44.7** | **2.6×** |")
    w("| 전단 | 5.9 | 6.2 | 1.05× |")
    w("| 합력 | 13.6 | 13.6 | 1.00× |")
    w()
    w("**합력이 Fz 에 끌려가는데도**(제곱합의 59 %) **Fz 보다 안정적이다.** 전단")
    w("항이 더해지면서 최솟값이 칼날 위에 서 있지 않게 되기 때문으로 보이는데,")
    w("사후 설명이고 검정한 것은 아니다. 실무적으로는 **인용할 한 수가 필요하면")
    w("Fz 가 아니라 합력을 쓰라**는 뜻이다 — Figure 5 의 힘 패널이 합력인 이유다.")
    w()
    w("> **왜 5 % 만 이런가.** 평탄점은 \"그 시편 최솟값의 105 % 안에 드는 가장")
    w("> 낮은 단\" 이다. 그 최솟값 자체가 시끄러운 argmin 이라, 문턱을 조일수록")
    w("> 통과 집합이 작아지고 seed 하나에 좌우된다. 110 % 는 통과 집합이 넓어")
    w("> 그렇지 않다.")
    w()
    w("> **그래서 철회한다.** 이 문서와 원고가 한때 \"허용치를 5 % 로 조이면 깊이")
    w("> 참조형 수직력이 열두 배 오른다\" 고 적었다. 그것은 판 하나의 성질이었고")
    w("> 두 번째 판에서 살아남지 못한다. **10 % 평탄점만 인용한다.**")
    w()
