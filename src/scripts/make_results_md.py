...
def main():
    ...
    w("| | 9DTact | DIGIT |")
    w("|---|---|---|")
    w("| cyl4 최소 MAE | **0.062 mm** @ 160×90 | 0.113 mm @ 1920×1080 |")
    w("| 해상도에 따른 변화 | 0.529 → 0.062 (**8.5 배**) | 0.121 → 0.111 (**평평**) |")
    w("| MAE < 0.1 mm 인 유닛 | 13 / 15 | **6 / 18** |")
    ...
    w()
    w("**실패 원인은 특정되지 않았다.** 열세 개 후보 변수 — 보정 자체의 오차, 기준영상")
    w("밝기·표준편차, 겔 산란, 헤일로 비, 자국 가장자리 대비, 축척과 그 신뢰 표시, 두께,")
    w("경도 — 를 유닛별 MAE 와 상관시켰는데 **유의한 것이 하나도 없다**(|ρ| ≤ 0.27,")
    w("p 0.27 ~ 1.00, n = 18). 겔이나 광학의 측정된 성질로는 설명되지 않으므로 원인은")
    w("파이프라인 내부(MLP 수렴, 보정 프레임 선택, 적분 경계)에 있을 가능성이 높다.")
    w("n = 18 이라 검정력이 약한 것도 사실이지만, ρ 가 전부 0.3 미만이라 큰 효과는 없다.")
    w()
