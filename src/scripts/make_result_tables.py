...
def grid(df, value, fmt="{:.2f}", note=None):
...
def table_ceiling(pr):
    ...
    D = pd.DataFrame(rows)
    save(D, grid(D, "ceiling_N", "{:.1f}", note="suspect"), "table_max_force", pr)
...
def table_resolution(pr):
    ...
    D = pd.DataFrame(rows)
    save(D, grid(D, "finest_centre_mm", "{:.2f}"), "table_spatial_resolution", pr)
    ...
    save(D2, grid(D2, "depth_range", "{}"), "table_resolved_depth_range", pr)
