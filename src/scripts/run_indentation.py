...
cal = _load("cal", ROOT / "src" / "scripts" / "prepare_tcp_calibration.py")
chk = _load("chk", ROOT / "src" / "scripts" / "robot_readonly_check.py")
xf = _load("xf", ROOT / "src" / "scripts" / "measure_sensor_base_transform.py")
mv = _load("mv", ROOT / "src" / "scripts" / "move_probe.py")
...
def _mm_per_px() -> float | None:
    cfg = yaml.safe_load(open(ROOT / "src" / "config" / "camera_config.yaml")) or {}
...
def phase_new(a) -> int:
    ...
    cam_cfg = yaml.safe_load(open(ROOT / "src" / "config" / "camera_config.yaml"))
    ft_cfg = yaml.safe_load(open(ROOT / "src" / "config" / "ft_config.yaml"))
...
def phase_sample(a) -> int:
    ...
    if not s.get("reference"):
        ...
        return 2
    rc = yaml.safe_load(open(ROOT / "src" / "config" / "robot_config.yaml"))
...
REGISTRY = ROOT / "src" / "config" / "sensor_registry.yaml"
...
def _scale_jacobian(result, theta_tol: float, max_rms: float):
    ...
    re-fitted offline by `src/scripts/refit_scale.py` through exactly this code
...
def main() -> int:
    ...
    ap.add_argument("--probe", default=None,
                    help="probe id from src/config/probes.yaml. Required by the "
...
def park_after_phase(rc: int) -> int:
    ...
    if phase not in GEL_PHASES or os.environ.get("VBTS_NO_EXIT_PARK") == "1":
        return rc
    try:
        rc_cfg = yaml.safe_load(open(ROOT / "src" / "config" / "robot_config.yaml"))
        ip = rc_cfg["robot"]["ip"]
        if "--ip" in argv:
        ...
        r = subprocess.run([sys.executable, str(ROOT / "src" / "scripts" / "move_probe.py"),
