...
왼팔(8×5 → 그 유닛의 최소)은 **해상도를 잃어서** 나빠지는 몫이고, 오른팔(1920×1080 →
최소)은 §2.3 에서 본 대로 **큰 입력에 과적합해서** 나빠지는 몫이다. 따로 재면 다른
변수를 따른다:

| | 두께 | 경도 |
|---|---|---|
| 왼팔 (해상도 손실) | ρ = +0.33, p = 0.20 | **ρ = +0.67, p = 0.003** |
| 오른팔 (과적합) | **ρ = +0.66, p = 0.004** | ρ = +0.11, p = 0.66 |

> 이 문서의 이전 판은 두 팔을 합친 U 깊이로 "두께를 따른다"(ρ = +0.57, p = 0.017)고
> 적었다. 그 상관은 **오른팔**에서 왔다 — 두꺼운 젤이 큰 입력에서 더 과적합한다는 뜻이지
> 해상도를 더 요구한다는 뜻이 아니다. 정정한다.

**해상도 요구는 경도가 정한다.** 왼팔 중앙값은 soft 0.017, medium 0.023, hard
...
왼팔의 무릎 위치를 유닛별로 잡으면 두께 순으로 32 / 48 / 80 px 이지만 p = 0.18 로
한 시드의 잡음에 묻힌다. 시드 sd 가 유닛당 0.0013–0.0066 N 이고 유닛 간 왼팔 차이가
