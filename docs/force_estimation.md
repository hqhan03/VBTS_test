...
실제로 쓰이는지** 물었다. `shape_reconstruction.md` §6 과 같은 12 개 16:9 크기를 써서 두
곡선을 나란히 놓을 수 있다.
...
잃는 양). **포화 해상도** = 최소에서 왼쪽으로 가다 3 시드 평균이 처음 `max(2 × 시드 sd, 최소의
...
| 유닛 | 최소 N | @ | 8×5 N | 해상도 손실 | 포화 해상도 | 시드 sd |
|---|---|---|---|---|---|---|
...
**포화 해상도는 젤과 무관하다.** 17 유닛의 포화 해상도가 32 / 48 / 80 px 에 4 / 8 / 5 로 모이고,
...
**포화 해상도 아래로 떨어질 때 잃는 양은 경도를 따르는 경향이 있다 — 확정은 아니다.** 해상도
...
| | `none` | `anti` |
|---|---|---|
| 포화 해상도 중앙값 (soft / medium / hard) | 64 / 48 / 48 | **80 / 80 / 64** |
...
포화 해상도가 **한 칸 위로**(48 → 64–80 px) 올라간다. 쌍극자만으로 전단을 읽으려면 두 엽을
...
|---|---|---|---|---|
| 9DTact (17 유닛) | 0.0253 N @ 80×45 | 0.0257 @ 80×45 | 0.0474 @ 320×180 | +53 / +23 / +10 % |
| DIGIT (18) | 0.0269 N @ 48×27 | 0.0333 @ 80×45 | 0.0611 @ 80×45 | +38 / +27 / +12 % |
...
단위는 **화소 밀도 R (px/mm²)** — 그 단으로 줄였을 때의 총 화소수를 그 유닛 카메라가
실제로 보는 면적으로 나눈 값이다. 원리끼리 견주는 표에서 픽셀 폭은 쓸 수 없다:
같은 1920 px 폭이 9DTact 에서 7 830, DIGIT 에서 10 389, DIGIT_Marker 에서
12 905 px/mm² 다(시야가 좁을수록 촘촘하다). 괄호 안이 실제로 학습한 사다리 단이다.

| 축 | 9DTact | DIGIT | DIGIT_Marker | 유의한 쌍 (Mann-Whitney) |
|---|---:|---:|---:|---|
| Fx | 4.9 (48 px) | 6.5 (48 px) | **50.4 (120 px)** | 마커가 가장 높다 |
| Fy | 13.6 (80 px) | 6.5 (48 px) | **50.4 (120 px)** | 마커가 가장 높다 |
| Fz | 13.6 (80 px) | **6.5 (48 px)** | 22.4 (80 px) | 세 원리가 거의 같다 |

유닛별 포화 해상도의 중앙값이다. **마커는 전단(Fx·Fy)의 포화 해상도가 가장 높고,
9DTact 는 Fz 가 가장 높다.** 축마다 어느 원리가 해상도를 더 요구하는지가 다르다.

밀도로 옮기면 마커의 전단 요구가 **더** 두드러진다 — 픽셀 폭으로는 2.5 배지만
(120 대 48), 밀도로는 **8~10 배**다(50.4 대 4.9·6.5). 마커 카메라의 시야가 더
좁아 같은 폭이 이미 더 촘촘하기 때문이다.

| 축 | 9DTact | DIGIT | DIGIT_Marker | 유의한 쌍 |
|---|---:|---:|---:|---|
| Fx | 0.0229 N | 0.0230 | 0.0248 | 없음 (p ≥ 0.17) |
| Fy | **0.0237** | 0.0309 | 0.0290 | 9DTact 가 나머지 둘보다 낮다 (0.046, 0.015) |
| Fz | **0.0422** | 0.0549 | 0.0710 | 셋 다 갈린다 (p ≤ 0.0007) |
...
**포화 해상도와 최소가 반대 방향이다.** DIGIT 은 **적은 픽셀로 자기 한계에 빨리 도달하지만
...
| | 해상도–오차 상관 | 최소 |
|---|---|---|
| 9DTact | ρ +0.084 p 0.80 (평평) | 48×27 |
| DIGIT | ρ −0.049 p 0.88 (평평) | 48×27 |
| **DIGIT_Marker** | **ρ −0.755 p 0.005** | **1920×1080** |
...
마커 점이 1.83 개뿐이라 변위 "장" 을 표본화하지 못하는데도 해상도가 도움이 된다면,
...
| | ρ | p |
|---|---:|---:|
| Fz 포화 해상도 vs 경도 | +0.10 | 0.550 |
| Fz 포화 해상도 vs 두께 | −0.06 | 0.721 |

**원리가 정하고 겔은 못 바꾼다.** 이것은 천장과 **다른 그림이다** — 천장은 9DTact 에서
겔 설계로 2.6 배 범위를 조절할 수 있다(`force_ceiling.md` §6.8). 두 결과를 함께
보고해야 한다.

유닛별 포화 해상도와 최소 MAE 는 `result/extra/data/force_knees.csv`, 그림은
...
7. 실무: `shape_reconstruction.md` §6.4 의 결론 — 픽셀 수보다 프레임률과 노출 — 이 힘
...
ImageNet 정규화, **기준 차분 없음**(§8.2). colour 가 Fz 는
...
> **왜 지운 쪽을 머리 결과로 썼는지 다시 본다 (2026-09-15).** 운전자 지적: 마커
> 쓰는 센서는 힘 추정에서 마커를 지우지 않는다. 문헌이 그렇다 — Marker
> Displacement Method 가 표준이고 마커를 **추적**해 전단을 읽는다. FEATS 는
> GelSight Mini 의 원본 이미지를 그대로 U-Net 에 넣는다(§8.1). 우리는 그
> 신호를 지우고 학습했고, 그 위에서 H9 판정을 냈다.
>
> 그래서 마커를 남긴 `colour`·`grey` 로 선택 아홉 유닛을 **축분리로** 다시
> 학습했다(2026-09-15). 결과는 `cross_principle.md` §3.5a 에 있다 — 요약하면
> **H9 의 방향은 살아남고 세기는 살아남지 않는다.** 마커 팔은 어느 전처리에서도
> 전해상도에서 나빠지지 않지만(비 0.85 ~ 0.95), 지운 판만 15 % **좋아지고**
> 남긴 판은 사실상 평평하다.
>
> 정리하면 이 절의 "짝지으면 차이가 없다" 는 **전해상도 한 점에서** 그렇다는
> 뜻이었고, 해상도를 가로질러 보면 전처리가 곡선의 기울기를 바꾼다. 마커 팔의
> 결과를 인용할 때는 **어느 전처리인지** 반드시 밝힐 것.

## 7.4 젤에 따라 포화 해상도가 달라지는가

이것이 캠페인의 원래 질문이다. 유닛별로 포화 해상도(오차가 자기 바닥에서 벗어나는 가장 작은 크기)과
바닥을 재고 경도·두께와의 순위 상관을 봤다(3 시드, `scripts/analyse_saturation.py`).

| | DIGIT | DIGIT_Marker |
|---|---|---|
| 전단 포화 해상도 ~ 두께 | ρ **−0.456** (p **0.057**), 중앙값 120 / 32 / 48 px | ρ −0.162 (p 0.52) |
| Fz 포화 해상도 ~ 두께 | ρ −0.095 (p 0.71) | ρ +0.020 (p 0.94) |
...
- **마커는 Fz 바닥이 두께를 따른다**(두꺼울수록 낮다). 포화 해상도는 어느 채널도 두께를 따르지 않는다.
...
2. **예외가 있고, 예외가 정보다.** `medium_1mm_r2` 와 `hard_1mm_r2` 는 전단이 Fz 위에 있고
   ...
   이고, 1 mm 겔은 전단 포화 해상도가 가장 큰(120 px) 집단이다(§4).
...
2. **포화 해상도는 2 배 간격 사다리에서 읽힌다.** 한 칸이 곧 2 배이므로 p 0.057 짜리 추세를
...
## 8. 문헌과 어디서 갈리는가 — 2026-09-10

> 2026-09-14 에 `method_vs_literature.md` 를 이 절로 옮겼다. 전부 이 문서가 쓴
> 전처리·학습 선택에 대한 것이라 따로 둘 이유가 없었다. 바깥에서 그 파일을
> 가리키던 참조는 이 절을 가리킨다.

이 문서의 §1 ~ §7 이 쓴 전처리·학습을, DIGIT/GelSight 로
...
### 8.3 다른 것 — 그리고 그 차이가 어디에 걸리는가

#### 8.3.1 시험 분할이 문헌보다 무르다 (가장 큰 차이)
...
영향을 받는 것은 "이 센서의 절대 정확도" 뿐이고, 그것을 주장하려면 정규화를 켜고 다시 재야 했다.
**수집 종료(2026-09-12)로 다시 재지 않으므로, 이 연구는 절대 정확도를 주장하지 않는다** —
겔 사이·원리 사이의 **상대 비교**만 주장한다. `methods.md` §10.0.

**재봤다 (2026-09-11).** DIGIT 의 레시피를 통째로 옮긴 팔(`--rep raw --input-norm imagenet`,
18 유닛)이 우리 방식(`colour`, 정규화 없음)보다 **나쁘다** — 전단 p 0.008, Fz p 0.054
(§7.3b). 즉 정규화를 안 켠 것이 우리 숫자를 부풀리고 있지는
...
지점은 정규화가 아니라 **기준 영상을 빼느냐**다.

#### 8.3.4 아무도 해상도를 쓸지 않는다
...
2 프레임 × 3 채널). §7.3 에서 재보니 **DIGIT 에서 색을 살리면
...
| 1b | ~~**마커 전용 전처리**(inpaint / flow)~~ **완료** | 둘 다 평범한 colour·grey 를 못 이겼다. 지우는 편이 추적하는 편보다는 나았다(p 0.039) | GPU 4.8 시간 |
| 2 | ~~**ImageNet 정규화**를 켜고 재학습~~ **완료** | 켠 쪽이 더 나빴다(§3.3). 정규화 단독의 몫을 가르는 `colour + ImageNet` 팔은 **돌리지 않았다** — 수집이 2026-09-12 에 끝나면서 닫혔다 | ~~GPU 2.4 시간~~ |
