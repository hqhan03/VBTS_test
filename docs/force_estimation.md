...
### 2.3b U 자는 17 유닛 중 16 개에 있다

중앙값 곡선 하나로는 "몇몇 유닛이 만든 모양"일 수 있다. 유닛마다 **U 깊이**를 재면
그렇지 않다: 양끝(8×5 와 1920×1080)의 평균에서 그 유닛의 최소를 뺀 값이다.

| 유닛 | 최소 크기 | 최소 N | U 깊이 N | 시드 sd 대비 |
|---|---|---|---|---|
| hard_3mm_r2 | 160×90 | 0.028 | **0.082** | 20.4× |
| medium_3mm_r2 | 160×90 | 0.032 | **0.064** | 15.9× |
| hard_3mm_r1 | 32×18 | 0.035 | **0.052** | 13.0× |
| soft_3mm_r2 | 80×45 | 0.026 | **0.037** | 9.3× |
| hard_2mm_r1 | 48×27 | 0.018 | **0.025** | 6.2× |
| hard_2mm_r2 | 80×45 | 0.023 | **0.025** | 6.2× |
| soft_1mm_r2 | 48×27 | 0.033 | **0.023** | 5.8× |
| medium_1mm_r2 | 80×45 | 0.032 | **0.022** | 5.5× |
| medium_3mm_r1 | 160×90 | 0.031 | **0.019** | 4.9× |
| soft_3mm_r1 | 160×90 | 0.038 | **0.018** | 4.4× |
| hard_1mm_r1 | 320×180 | 0.012 | **0.018** | 4.4× |
| hard_1mm_r2 | 854×480 | 0.016 | **0.017** | 4.3× |
| soft_2mm_r2 | 80×45 | 0.023 | **0.015** | 3.8× |
| medium_1mm_r1 | 80×45 | 0.023 | **0.015** | 3.7× |
| medium_2mm_r2 | 80×45 | 0.027 | **0.011** | 2.7× |
| soft_2mm_r1 | 320×180 | 0.022 | **0.008** | 2.0× |
| soft_1mm_r1 | 80×45 | 0.023 | **0.006** | 1.6× |

**16 / 17 유닛에서 U 깊이가 시드 산포(0.0040 N)의 2 배를 넘는다.** 중앙값은 0.019 N
으로 시드의 **4.9 배**다. 예외는 `soft_1mm_r1` 하나(1.6 배)다. 그리고 최소가 놓이는
자리도 흩어지지 않는다 — **80×45 가 7 유닛, 160×90 이 4 유닛**으로 17 중 11 이
80–160 px 폭에 모인다.

**U 의 두 팔은 다른 것을 잰다.** 양끝을 합쳐 "U 깊이"로 부르면 두 효과가 섞인다.
**해상도 손실**(8×5 에서의 오차 − 그 유닛의 최소)은 그림을 줄여서 나빠지는 몫이고,
**과적합 손실**(1920×1080 에서의 오차 − 최소)은 §2.3 에서 본 대로 큰 입력에 과적합해서
나빠지는 몫이다. 따로 재면 다른 변수를 따른다:

| | 두께 | 경도 |
|---|---|---|
| 해상도 손실 | ρ = +0.33, p = 0.20 | **ρ = +0.67, p = 0.003** |
| 과적합 손실 | **ρ = +0.66, p = 0.004** | ρ = +0.11, p = 0.66 |

> 이 문서의 이전 판은 두 팔을 합친 U 깊이로 "두께를 따른다"(ρ = +0.57, p = 0.017)고
> 적었다. 그 상관은 **과적합 손실**에서 왔다 — 두꺼운 젤이 큰 입력에서 더 과적합한다는 뜻이지
> 해상도를 더 요구한다는 뜻이 아니다. 정정한다.

**해상도 요구는 경도가 정한다.** 해상도 손실 중앙값은 soft 0.017, medium 0.023, hard
0.033 N 이고 `hard_3mm_r2` 가 0.119 N 으로 가장 깊다. 단단한 젤은 전단 변위가 작아
자국의 비대칭이 얇고 날카롭다 — 공간 주파수가 높다 — 부드러운 젤은 넓고 완만하다.
전단 신호 자체의 대역폭을 재면(`scripts/imprint_bandwidth.py`: 전단 프레임에서 **같은
사이클의 최소 전단 프레임**을 뺀 것의 스펙트럼, 같은 Fz 의 normal 프레임 두 장으로 잰
잡음 스펙트럼을 차감, 0–3 cycles/mm 로 한정) 90 % 에너지가 **0.30–0.50 cycles/mm** 안에
있다. 19 mm 시야에 Nyquist 로 **11–19 화소**라는 뜻이고, 관측된 전단 무릎(16–160 px
폭)의 아래쪽과 같은 자리다. 경도와의 상관은 ρ = +0.30, p = 0.24 로 **유의하지 않다** —
스펙트럼을 0.1 cycles/mm 칸으로 읽어 값이 0.3 / 0.4 / 0.5 로만 떨어지는 거친 추정이라,
1.7 배의 범위 안에서 경도 순서를 가릴 힘이 없다. (이 절의 이전 판은 스크래치 계산에서
0.30–0.80, ρ = +0.46, p = 0.06 을 인용했다. 그 0.80 은 `hard_3mm_r2` 의 표본 하나였고
재현 스크립트에서는 나오지 않는다. 정정한다.) 해상도 손실 쪽의 경도 의존성(p = 0.003)은 학습 결과
쪽 증거이고, 대역폭 측정은 그것과 **방향은 같되 독립적으로 확정하지는 못한다.**

같은 방법으로 잰 **normal 자국**의 대역폭은 0.20–0.30 cycles/mm(Nyquist 8–11 화소)이고
두께와 ρ = −0.57, p = 0.017 — 얇은 젤일수록 자국이 날카롭다. 범위는 1.5 배뿐이다.
`data/9DTact/imprint_bandwidth.csv`.

해상도 손실의 무릎 위치를 유닛별로 잡으면 두께 순으로 32 / 48 / 80 px 이지만 p = 0.18 로
한 시드의 잡음에 묻힌다. 시드 sd 가 유닛당 0.0013–0.0066 N 이고 유닛 간 해상도 손실 차이가
0.01–0.12 N 이므로, **시드를 셋만 돌리면 갈린다.**
...
- **이 판의 전단 테스트 집합은 너무 작았다.** 사이클 경계를 `searchsorted(누적, 0.85) + 1`
  ...
  **12 개(2.4 %)** 로 시험됐고, 그 유닛의 전단 MAE 는 시드에 따라 0.020 ↔ 0.040 으로
  ...
  51/25/24 가 통째 사이클로 가능한 가장 정직한 분할이다. **이 문서의 수치는 아직 옛
  분할로 잰 것이다** — 특히 §2.3 · §2.3b 의 전단 값은 유닛에 따라 12–87 프레임 위에
  서 있으므로, 새 분할·시드 3 개로 다시 돌린 결과가 나오면 교체한다.
