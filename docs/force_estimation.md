...
1. **힘 추정에 1920×1080 은 필요 없다.** 1920×1080 부터 48×27 까지 성능 차이가
   시드 산포(±0.022 N) 안에 있다. 형상 복원이 80×45 까지 유지되는 것
   (`shape_vs_resolution.md`)과 같은 방향이고, 힘 쪽이 더 관대하다.
2. **오차의 바닥은 카메라가 아니라 라벨이다.** 중앙값 0.052 N 은 F/T 읽기 자신의
   잡음(0.028 N)의 1.8 배이고, 유닛별로 둘이 함께 움직인다(ρ = +0.55).
   더 좋은 힘 추정을 원하면 카메라가 아니라 **라벨의 평균 시간**을 늘려야 한다.
3. **망은 스칼라보다 1.55 배 낫다.** 이미지 전체 밝기가 힘의 대부분을 담지만
   (스칼라만으로 R² 0.94), 공간 구조에 남은 정보가 있다.
4. **젤 두께·경도는 힘 추정 성능을 정하지 않는다.** 복제 간 차이가 사양 간 차이보다 크다.
5. 실무: 힘·형상용 카메라는 픽셀 수보다 **프레임률과 노출**을 사야 한다는
   `shape_vs_resolution.md` §4 의 결론이 힘 쪽에서도 유지된다.
