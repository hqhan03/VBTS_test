...
### 결과 — 18 유닛 중 15 개가 0.1 mm 밑 (2026-09-13, 2026-09-14 정정)
...
| | 고치기 전 | 고친 뒤 |
|---|---|---|
| `cyl4` 최소 MAE | 0.113 mm @ 1920×1080 | **0.043 mm** |
| MAE < 0.1 mm 인 유닛 | 6 / 18 | **15 / 18** |
| 유닛 안의 상관 (중앙) | — | **+0.980** (16 / 18 이 > 0.9) |

> **두 번째 정정 (2026-09-14).** 이 자리에 "해상도 의존성이 나타났다 — 8×5 의
> 0.170 mm 에서 80×45 의 0.043 mm 까지 3.9 배" 라고 적었다. **그 의존성의 대부분은
> 두 번째 결함이 만든 것이었다.** `predict_depth` 의 접촉 마스크가 3×3 열림을
> **픽셀 단위로 고정**해 두어, 저해상도에서 커널이 자국보다 커졌다 — 16×9 에서
> 자국 18 px 이 열림 뒤 **0 px** 이 되어 기울기가 전부 0 으로 강제됐고, 깊이가
> 통째로 0 이 됐다. 32×18 도 49 → 9 px 로 82 % 를 잃고 있었다. 8×5 에서 프레임의
> 80 %, 16×9 에서 74 %, 32×18 에서 52 % 가 **정확히 0 을 예측**했고, 8×5 의 오차
> 중앙값 0.1425 mm 는 참 깊이 중앙값과 자릿수까지 같았다 — 복원 오차가 아니라
> "아무것도 못 찾았다" 였다.
>
> 커널을 폭에 비례시켜(1920 px 에서 3 px, 그 아래로는 1 px 이면 걸지 않는다)
> 다시 냈다. **1920×1080 의 값은 0.0540 mm 로 한 자리도 바뀌지 않았고**
> 160×90 이상의 중앙 차이는 0.002 mm 다. 바뀐 것은 저해상도 팔뿐이다.
>
> 같은 판에서 **`eval_unit` 이 `load_ref()` 를 건너뛰고 `reference.png` 를 직접
> 읽고 있던 것**도 고쳤다. 바로 위에서 "쓰면 안 된다" 고 적어 둔 그 파일이다.
> 원래 결과를 낸 구동부가 저장소에 없어서(`main()` 이 빈 껍데기였다) 이 함수가
> 검증 없이 남아 있었다. 이제 `digit_shape_eval.py` 가 구동부다.

해상도별 |오차| 중앙 (mm), 18 유닛:

| 해상도 | 8×5 | 16×9 | 32×18 | 48×27 | 80×45 | 160×90 | 320×180 | 1920×1080 |
|---|---:|---:|---:|---:|---:|---:|---:|---:|
| `cyl4` | 0.129 | 0.081 | 0.055 | 0.052 | 0.054 | 0.052 | 0.051 | 0.048 |
| `cube4` | 0.100 | 0.064 | 0.060 | 0.064 | 0.064 | 0.063 | 0.062 | 0.058 |

**곡선이 32×18 까지 평평하다.** `cyl4` 가 1920×1080 의 0.048 mm 에서 32×18 의
0.055 mm 로, 여섯 단을 내려오는 동안 15 % 밖에 나빠지지 않는다. 나빠지는 것은
16×9(0.081)와 8×5(0.129)뿐이다. 유닛별 포화 해상도의 중앙은 **R ≈ 2.9 px/mm²
(32×18)** 로, 옛 판의 80×45 보다 **네 단** 아래다.

> **두 파이프라인 모두 훨씬 낮은 해상도를 견딘다.** 같은 날 9DTact 쪽에서도
> 같은 종류의 결함을 찾아 고쳤다(§6.5a) — 7×7 가우시안 셋이 출력 픽셀 단위로
> 고정돼 있었다. 고친 뒤 포화 해상도는 9DTact R ≈ 5.5, DIGIT R ≈ 2.9 로
> **1.9 배** 안에 든다. 2026-09-13 판에서 둘 다 80×45 (R 14~18) 로 보였던 것은
> 우연이었다 — 각 파이프라인의 서로 다른 필터가 저해상도를 각자 망가뜨리고
> 있었고, 그 두 붕괴가 비슷한 자리에서 일어났을 뿐이다.
...
## 6. 해상도 쓸기 — 형상에 픽셀이 얼마나 필요한가

> 2026-09-14 에 `shape_vs_resolution.md` 를 이 절로 옮겼다. 형상 복원의 해상도
> 의존성은 이 문서가 답하는 것과 같은 질문이라 문서를 나눌 이유가 없었다.
> 바깥에서 `shape_vs_resolution.md` 를 가리키던 참조는 이 절을 가리킨다.
...
#### 6.4a DIGIT 도 같은 자리에 포화 해상도가 있다 (2026-09-13)

이 문서는 9DTact 의 밝기→깊이 방식에 대한 것이다. 2026-09-13 에 DIGIT 의 광도
스테레오 파이프라인으로 같은 쓸기를 돌렸더니(`shape_reconstruction.md` §5)
**두 파이프라인 다 예전에 적은 것보다 훨씬 낮은 해상도를 견딘다** (2026-09-14
정정). DIGIT 의 포화 해상도는 R ≈ 2.9 px/mm² (32×18), 9DTact 는 R ≈ 5.5 (48×27)
로 **1.9 배** 안에 든다. 2026-09-13 판에서는 둘 다 80×45 (R 14~18) 였다.

> **둘 다 같은 종류의 결함이었고, 원인은 서로 달랐다.** DIGIT 은 접촉 마스크의
> 3×3 열림(§5), 9DTact 는 7×7 가우시안 셋(§6.5a) 이 **픽셀 단위로 고정**돼
> 저해상도에서 자국보다 커졌다. 두 붕괴가 우연히 비슷한 자리에서 일어나
> "같은 자리에 포화한다" 로 읽혔다. 고치고 나니 둘 다 그보다 서너 단 아래다.

**두 방식의 절대 정확도를 비교하지는 말 것.** DIGIT 은 0.010 ~ 0.578 mm 를 평가하고
9DTact 는 그보다 깊은 구간(구 최심 중앙 0.761 mm)을 평가한다. 같은 자가 아니다.
...
### 6.5a 커널이 픽셀 단위였다 — 저해상도 팔을 다시 냈다 (2026-09-14)

이 절의 표와 결론은 `reconstruct()` 가 7×7 가우시안을 **출력 픽셀 단위로 세 번**
거는 판에서 나온 것이었다(차영상에 한 번, 깊이맵에 두 번). §6.5 가 그것을 한계로
적어 두었지만("커널을 mm 로 고정하면 낮은 해상도 쪽의 깊이 붕괴가 더 늦게 올 수
있다") 고치지는 않았다. 고쳤다 — 커널을 축소 배율로 나누고 3 px 밑으로 내려가면
걸지 않는다. **1920×1080 에서는 7 그대로라 전해상도 값이 바뀌지 않는다.**

`cyl4` MAE 중앙(17 유닛):

| 폭 | 320 | 160 | 80 | 48 | 32 | 16 | 8 |
|---|---:|---:|---:|---:|---:|---:|---:|
| 옛 | 0.064 | 0.069 | 0.087 | 0.155 | 0.251 | 0.441 | 0.528 |
| 새 | 0.064 | 0.063 | **0.062** | **0.072** | 0.113 | 0.192 | 0.206 |

**"형상 정보는 80×45 까지 살아 있다" 를 정정한다** — 48×27 까지다. 유닛별 포화
해상도의 중앙은 R ≈ 5.5 px/mm² (48×27) 로, 옛 판의 R ≈ 55 (160×90) 보다 **열 배**
아래다. 곡선의 오른쪽 팔(전해상도가 손해라는 것)은 그대로다.

> 같은 판에서 다섯 유닛(`medium_1mm_r2`, `medium_3mm_r1`, `medium_3mm_r2`,
> `soft_1mm_r2`, `soft_3mm_r1`)의 `scale_source` 가 **borrowed → own** 으로
> 바뀌었다. 저장돼 있던 CSV 가 그 유닛들의 자체 축척 보정이 들어오기 전 것이었다.
> 커널 수정과 무관한 별개의 개선이고, 그래서 전해상도 값도 그 다섯에서만 움직인다.
...
/usr/bin/python3 src/scripts/analyse_shape_resolution.py           # §6 의 12 단 쓸기
```
