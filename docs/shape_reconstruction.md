...
**포화 해상도가 80×45 에 있고 그 위로 평평하다** — 9DTact 와 같은 픽셀 폭이다(§3).
화소 밀도로는 DIGIT 18.0, 9DTact 13.6 px/mm² 로 1.33 배 차이인데, 이는 2 배 간격
사다리 한 칸 안이다.
...
**포화 해상도가 80×45 로 같은 픽셀 폭**에 있다 — `cyl4` MAE 가 8×5 의 0.170 mm 에서
80×45 의 0.043 mm 까지 3.9 배 좋아지고 그 위로 평평하다.

> **같은 폭이 같은 밀도는 아니다.** 80×45 는 9DTact 에서 13.6, DIGIT 에서
> 18.0 px/mm² 다 — DIGIT 의 시야가 좁아 1.33 배 촘촘하다. 2 배 간격 사다리의 한
> 칸(4 배)보다 작으므로 "같은 자리" 라는 읽기는 남지만, **두 원리의 요구량을
> 숫자로 견줄 때는 밀도를 쓸 것.**
