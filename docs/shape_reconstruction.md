...
> **그러므로 "DIGIT 도 9DTact 와 같은 자리에 포화 해상도가 있다" 는 주장을
> 철회한다.** 9DTact 는 R ≈ 54, DIGIT 은 R ≈ 2.9 로 **열여덟 배** 차이다. 두
> 파이프라인이 같은 자리에 있는 것처럼 보였던 것은 DIGIT 쪽 마스크가 저해상도를
> 망가뜨리고 있었기 때문이다.
...
**두 파이프라인은 같은 자리에 있지 않다** (2026-09-14 정정). DIGIT 의 포화 해상도는
R ≈ 2.9 px/mm² (32×18) 이고 9DTact 는 R ≈ 54 (160×90) 로, **열여덟 배** 차이다.

> **2026-09-13 에는 둘 다 80×45 라고 적었다.** 그것은 DIGIT 쪽 접촉 마스크가
> 저해상도에서 자국을 지워 곡선의 왼팔을 들어 올렸기 때문이다(§5 의 정정).
> 고치고 나니 DIGIT 은 32×18 까지 평평하다. 광도 스테레오가 밝기→깊이 조회표보다
> **낮은 해상도를 견딘다** — 조회표는 화소마다 독립적으로 깊이를 읽지만 광도
> 스테레오는 기울기를 적분하므로, 화소 몇 개만 있어도 모양이 남는다.
