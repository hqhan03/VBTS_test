...
잘 되는 6 유닛은 MAE 0.038 ~ 0.092 mm 로 9DTact 와 같은 수준이다. 나머지는 0.4 mm
이상까지 벌어진다.

**실패 원인은 특정되지 않았다.** 열세 개 후보 — 보정 자체의 오차, 기준영상 밝기·
표준편차, 겔 산란, 헤일로 비, 자국 가장자리 대비, 축척과 그 신뢰 표시, 두께, 경도 —
를 유닛별 MAE 와 상관시켰는데 **유의한 것이 하나도 없다**(|ρ| ≤ 0.27, p 0.27 ~ 1.00,
n = 18). 겔이나 광학의 측정된 성질로 설명되지 않으므로 "더 좋은 보정 자료로 다시" 가
통하지 않는다.

**그리고 이 지표로는 해상도 의존성을 볼 수 없다.** 8 px 에서도 1920 px 와 오차가
같은데, 8×5 픽셀로 형상을 복원할 수는 없다. 지표가 **최대 깊이 하나**뿐이고 그 값은
사실상 **색 변화의 적분량**이라, 면적평균 다운샘플링이 그것을 정확히 보존하기
때문이다 — `force_estimation.md` 가 Fz 에서 확인한 성질과 같다. 9DTact 쪽 지표는
깊이만이 아니라 크기·평탄도·기울기를 함께 보므로 의존성이 드러난다.

> **그러므로 DIGIT 의 형상 복원은 결과가 아니라 한계로 보고한다.** 센서의 성질이
> 아니라 이 시제품의 한계를 재고 있다. 닫으려면 (a) 9DTact 와 같은 **형상 충실도
