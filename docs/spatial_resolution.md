...
### 4.3 두께·경도별 (센서×간격 조합 68 개 중)

| 두께 | 분해 | 대칭성 sym 중앙값 | | 경도 | 분해 |
|---|---|---|---|---|---|
| 1 mm | 8 / 24 | 0.70 | | soft | 12 / 24 |
| **2 mm** | **15 / 20** | **0.88** | | medium | 7 / 20 |
| 3 mm | 5 / 24 | 0.66 | | hard | 9 / 24 |

2 mm 가 압도적으로 좋아 보이지만 **2 mm 가 접촉 대칭성도 가장 좋다.** 두 기둥이 같은
깊이로 들어간 정도(sym)가 두께와 함께 움직이므로, **두께 효과와 안착(seating) 효과가
분리되지 않는다.** 이 표에서 "2 mm 젤이 가장 분해능이 좋다"고 결론내리면 안 된다 —
sym 을 공변량으로 넣은 분석이 먼저다(미완).
