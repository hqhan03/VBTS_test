...
| DIGIT 전단 무릎 ~ 두께 | 1 mm | 2 mm | 3 mm | ρ | p |
|---|---|---|---|---|---|
| 2 배 사다리 (`colour`) | 48 px | 48 px | 40 px | +0.014 | 0.957 |
| **√2 사다리 (`colour`)** | **40 px** | **56 px** | **40 px** | **−0.047** | **0.854** |

**더 촘촘한 사다리가 같은 답을 준다 — DIGIT 에서 두께 효과는 없다.** 무릎 자체는
40–56 px 로 2 배 사다리의 48 px 와 일치하므로, **무릎이라는 양은 견고하고 겔 의존성만
없다.** 사다리 간격이 원인이라는 변명은 이것으로 닫힌다.

**그래서 "포화 해상도가 겔에 따라 달라지는가" 에 대한 답은 DIGIT 에서 '아니오' 다.**
남는 것은 `grey` 하나에서 나온 −0.456 (p 0.057) 인데, 같은 프레임을 더 나은 표현
(§4.1)과 더 촘촘한 사다리로 읽으면 두 번 다 사라진다. 방향(모든 ρ ≤ 0)과 학습 없는 두
측정(§3.6, §3.9)은 그대로 남지만, **학습된 무릎으로는 이 효과를 보일 수 없다.**
