...
### 3.8 두 점 분해: 첫 DIGIT 유닛이 가장 좁은 간격을 모든 깊이에서 분해한다 (진행 중)

`data_wishlist.md` #0 의 수집을 시작했다. 첫 유닛 **DIGIT_hard_3mm_r2, pair025**
(⌀1 mm 원기둥 두 개, 가장자리 간격 0.25 mm → 중심간 1.25 mm), 깊이 사다리 0.1–0.5 mm,
`scripts/analyse_resolution.py` 의 **미리 고정된** 기준으로 판정:

| 깊이 (mm) | dip | Rayleigh (≥0.265) | MTF | Digit 360 (MTF ≥ 0.50) |
|---|---|---|---|---|
| 0.10 | 0.831 | 분해 | 0.711 | 분해 |
| 0.20 | 0.988 | 분해 | 0.976 | 분해 |
| 0.30 | 1.000 | 분해 | 1.000 | 분해 |
| 0.40 | 1.000 | 분해 | 1.000 | 분해 |
| 0.50 | 1.000 | 분해 | 1.000 | 분해 |

**가장 얕은 0.1 mm 에서 이미 완전히 분리**되고, 0.3 mm 부터는 골이 바닥까지 내려간다(dip 1.000).
같은 칸의 9DTact(hard 3 mm)는 **네 간격 모두 분해하지 못했다**(`n_resolved` 0). 미리 등록한 예측 1
("DIGIT 이 9DTact 보다 더 많은 간격을 분해한다")과 방향이 같고, 크기는 예측보다 크다 — DIGIT 의 한계는
1.25 mm **아래** 어딘가이고 현재 프로브 세트로는 잴 수 없다.

**부수적으로 축척이 검증됐다.** 두 자국의 중심간 거리를 재면 1.19–1.33 mm 로, 설계값 1.25 mm 와 5 % 안에서
맞는다(`scripts/pair_check.py`). pair 프로브는 **간격이 알려진 자**이므로 어떤 간격이든 축척 검증을 겸한다 —
`data_wishlist.md` #5 가 pair100 을 요구했지만 pair025 로도 똑같이 된다.

**프로토콜에 대한 함의.** DIGIT 이 가장 좁은 간격을 여유 있게 분해하므로 **050/075/100 은 더 쉬운 문제라
정보가 없다**. 남는 질문은 두 가지다: (a) 모든 DIGIT 유닛이 그러한가 — 특히 가장 불리할 3 mm soft, (b)
마커 유닛에서 점이 자국을 가려 무너지는가(§3.5: 점 지름 1.09 mm 대 중심간 1.25 mm). 그러므로 pair025 를
36 유닛 전부에 돌리고, 나머지 간격은 돌리지 않는다. DIGIT 의 진짜 한계를 재려면 **간격 ≤ 0.1 mm 짜리
새 pair 프로브**가 필요하다.

**둘째 유닛이 첫 결론을 뒤집었다 (그리고 사다리를 고쳤다).** 같은 칸의 **DIGIT_hard_3mm_r1** 은 0.1–0.5 mm
에서 하나도 분해하지 못했다(dip 0.08 → −0.27, `sym` 0.71 → 0.26). 원인은 센서가 아니라 **압입 부족**이었다:
이 유닛의 영점은 힘 적합과 이미지 개시가 0.26 mm 어긋났고, 그만큼 늦게 접촉해 "0.5 mm" 가 실제로는 약 0.3 mm
였다(0.1 mm 단에서 힘 0.027 N, 자국 0 px — r2 는 같은 단에서 0.137 N, 38520 px). 같은 유닛에 0.6–0.9 mm 단을
더 찍자 **0.6 mm 부터 분해**되고(dip 0.48 → 0.70) `sym` 이 0.97 로 올라간다 — 두 기둥이 균등하게 눌린다.

| 유닛 | 분해 시작 | 그때의 힘 | 2 N 접촉 응답 (Pass B) | 압흔 대역폭 |
|---|---|---|---|---|
| hard_3mm_r2 | 0.10 mm (첫 단) | 0.137 N | 6.61 레벨 | 0.5 cyc/mm |
| hard_3mm_r1 | 0.60 mm | 0.391 N | 5.03 레벨 | 0.4 cyc/mm |

두 유닛의 차이는 이 pass 에서 처음 나온 것이 아니다 — Pass B 의 접촉 응답(5.03 대 6.61)과 압흔 대역폭
(0.4 대 0.5)이 이미 r1 을 약한 쪽으로 찍고 있었다. 초점은 같다(라플라시안 분산 19.35 대 19.37). 즉 **같은
사양의 복제 두 개가 분해에 필요한 힘에서 3 배 차이**가 나며, 이는 우리가 재려는 원리 간 차이만큼 크다.
그러므로 (a) 표준 사다리를 **겔 백스톱이 허용하는 만큼 깊게**(1 mm 겔 0.1–0.6, 나머지 0.1–0.9) 바꿨고,
(b) 유닛별 결론은 18 유닛이 다 모이기 전에는 내리지 않는다.

**DIGIT 18/18 이 pair025 를 분해했다 → pair010 을 새로 만들었다.** 18 유닛 전부가 중심간 1.25 mm 를
분해했고 대부분 첫 0.1 mm 단에서였다. 그러므로 DIGIT 의 한계는 이 프로브 세트 아래에 있다. 운영자가
**pair010**(⌀1 mm 원기둥 두 개, 가장자리 간격 0.10 mm → **중심간 1.10 mm**, 팁 끝점은 기존과 공통 설계)을
장착해 `config/probes.yaml` 에 추가했다. 첫 유닛 **DIGIT_soft_1mm_r1** 은 **첫 단(0.1 mm)부터 Rayleigh 분해**
(dip 0.573), 0.3 mm 부터 Digit 360 도 통과, 0.6 mm 에서 0.999 다. 자국 두 덩어리의 중심간 거리는 깊은 단에서
**1.10–1.12 mm** 로 설계값과 일치해 프로브 기하와 축척이 다시 확인됐다.

**`sym` 은 DIGIT 에서 기울기 지표가 아니다.** pair010 에서 `sym` 이 0.43–0.49 로 떨어지지만 이미지에는 두
자국이 모두 찍혀 있다. DIGIT 은 측면 조명이라 같은 깊이의 두 기둥이라도 조명 쪽을 향한 쪽이 더 밝다 —
봉우리 높이의 비는 조명 방향을 읽는 것이지 하중 분배를 읽는 것이 아니다. 기울어진 마운트는 `sym` 이 아니라
**덩어리가 하나만 남는지**로 판정해야 한다(`scripts/pair_check.py`, `scripts/marker_occlusion.py` 와 같은
연결요소 계산). 9DTact 는 조명이 다르므로 이 주의가 적용되지 않는다.

**기울어진 마운트 검사.** `analyse_resolution.py` 의 `sym` 열(두 봉우리 높이의 비)이 이 유닛에서 0.77–0.87
로, 두 기둥이 모두 눌렸다. 한쪽만 눌리면 봉우리가 하나뿐이라 `sym` 이 아니라 dip 자체가 사라진다.
`scripts/pair_check.py` 는 자국 덩어리 두 개의 면적·중심간 거리를 따로 찍어 같은 것을 확인한다.
...
**한계.** 1 seed 다. 9DTact 에서 seed 표준편차는 Fz 0.005 N 수준이었고 무릎은 2 배 간격 사다리에서 읽히므로
한 칸이 곧 2 배다. 전단 무릎의 p 0.051 은 **경계선**이고, 3 seed 패스(큐에 있음)가 이것을 확인하거나
뒤집을 것이다. Fz 해상도 손실의 p 0.0014 는 더 견고하다.
