...
**표현을 바꾸면 사라진다 (2026-09-11).** 위 검정은 전부 `grey` 스윕에서 한 것이다. 같은
18 유닛·같은 3 seed 를 `colour` 스윕으로 다시 재면:

| 전단 무릎 ~ 두께 | 1 mm | 2 mm | 3 mm | ρ | p |
|---|---|---|---|---|---|
| `grey` (위의 검정) | 120 px | 32 px | 48 px | **−0.456** | **0.057** |
| `colour` | 48 px | 48 px | 40 px | **+0.014** | **0.957** |

**입력 표현만 바꿨는데 결과가 없어진다.** 그리고 §4.1 에서 `colour` 가 DIGIT 의 더 나은
표현임을 데이터로 확인했다(Fz·전단 모두 오차가 낮고, 전단은 p 0.008 로 유의). 즉 캠페인의
표제 결과가 **더 나쁜 표현에서만** 성립한다.

**그래서 "포화 해상도가 겔에 따라 달라지는가" 에 대한 이 캠페인의 답은 '아니오' 쪽으로
정정한다.** grey 에서 본 −0.456 (p 0.057) 은 애초에 유의 문턱에 걸쳐 있었고, 더 정확한
표현에서 재현되지 않는다. 무릎을 예측하는 변수는 §3.2 의 53 유닛 편상관에서도 없었다
(DIGIT 안의 `mu` 하나 제외).
