...
**`mu` → 전단 무릎 ρ −0.820, q 0.005**(18 유닛, 150 검정). 마찰이 큰 유닛일수록 전단 무릎이
낮은 크기에서 온다. 무릎을 예측하는 변수가 하나라도 나온 것은 이번이 처음이고, 한 원리
...
| H3 대역폭 → 무릎 | **기각** | `bw_shear_f90` → 전단 무릎 +0.10 p 0.50 |
| H4 접촉 반경 → Fz 무릎 없음 | **예측대로** | `bw_normal_f90` → Fz 무릎 +0.17 p 0.22 |
...
**무릎**을 예측하는 변수는 원리 통제 후에도 없다(DIGIT 안의 `mu` 하나 제외). (iv) 이 절의
...
> **정정 진행 중 (2026-09-13).** "실효 배치 64" 가 **세 해상도에서 사실이 아니었다** —
> 640×360 에서 44, 854×480 에서 72, 1280×720 에서 66 이었다.
> `accum = round(batch / micro)` 의 반올림 때문이고 나머지 아홉 단은 정확히 64 다.
> 미세배치가 배치를 나누도록 고쳐 **그 세 단만** 다시 학습하는 중이다.
...
| | 해상도–오차 상관 | 무릎 | 최소 |
|---|---|---|---|
| 9DTact | ρ +0.070 p 0.83 (평평) | 48×27 | 80×45 |
| DIGIT | ρ +0.021 p 0.95 (평평) | 48×27 | 80×45 |
| **DIGIT_Marker** | **ρ −0.727 p 0.007** | **1920×1080** | 1920×1080 |
...
- **크기와 유의성은 재현되지 않는다.** 어느 표현에서 유의해지는지가 원리마다 다르다는 것은,
  이 무릎 추정이 그 차이를 견딜 만큼 안정적이지 않다는 뜻이다.

무릎을 2 배 간격 사다리에서 읽는 것이 여기 한몫한다 — 120 px 와 48 px 는 두 칸 차이일
뿐이다. 그래서 **√2 간격 사다리로 다시 물었다**(16, 24, 32, 40, 48, 64, 80, 113, 160,
226, 320 px; `colour`, 3 seed, `sqrt2_knee.py`).

| 전단 무릎 ~ 두께 | 1 mm | 2 mm | 3 mm | ρ | p |
|---|---|---|---|---|---|
...
**두 원리 모두 사라진다.** 마커에서 유일하게 유의했던 −0.609 는 무릎이 400 / 854 / 48 px
로 벌어진 위에서 계산된 값이고, √2 사다리에서는 44 / 36 / 44 px 로 좁고 평평해지면서
−0.113 (p 0.656) 이 된다. **불안정한 무릎 추정이 만든 허상이었다.**

무릎이라는 양 자체는 견고하다 — DIGIT 40–56 px, 마커 36–44 px 로 2 배 사다리의 48 px
근처에 있다. 흔들린 것은 무릎의 값이 아니라 **그것이 겔을 따라간다는 주장**이다.

**그래서 "포화 해상도가 겔에 따라 달라지는가" 에 대한 이 캠페인의 답은 '아니오' 다.**
방향(모든 ρ ≤ 0)과 학습 없는 두 측정(§3.6 압흔 대역폭 24 → 17 px, §3.9 광학 번짐
1.0 → 3.2 mm)은 그대로 남지만, **학습된 무릎으로는 이 효과를 보일 수 없다.**

**√2 사다리에서 새로 나온 두 신호는 기록만 한다.** 마커의 전단 무릎이 경도를 따르고
(72 / 40 / 36 px, ρ −0.591, **p 0.010**), Fz 무릎이 두께를 따른다(28 / 40 / 44 px,
...
무릎을 2 배 간격 사다리에서 읽는 것도 이 취약함에 한몫한다 — 한 칸이 곧 2 배이므로
...
무릎은 **최소 MAE 의 110 % 안에 드는 가장 낮은 해상도**로 정의한다.

| 축 | 9DTact 무릎 | DIGIT 무릎 | p | 9DTact 최소 MAE | DIGIT | p |
|---|---:|---:|---|---:|---:|---|
| Fx | 48 px | 48 px | 0.758 | 0.0229 N | 0.0232 | 0.754 |
| **Fy** | **80** | **40** | **0.003** | **0.0237** | 0.0310 | **0.020** |
| **Fz** | **160** | **64** | **0.015** | **0.0416** | 0.0557 | **0.00005** |
...
**겔은 무릎을 못 가른다**: Fz 무릎 대 경도 ρ +0.10 (p 0.55), 대 두께 ρ −0.06 (p 0.72).
