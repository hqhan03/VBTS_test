...
| 성능 | `fz_best/knee/res_loss`, `sh_best/knee/res_loss` | 해상도 스윕 (`analyse_saturation`) | 최소 오차, 포화 해상도, 해상도 손실 |
...
- **H3 대역폭 → 포화 해상도.** `bw_normal_f90` 이 `fz_knee` 를, `bw_shear_f90` 이 `sh_knee` 를 정한다(Nyquist). 예상: 상관은 있으되 범위가 1.5–1.7 배라 한 칸 안.
- **H4 접촉 반경 → 포화 해상도: 없음.** `a_2N_mm` 은 `fz_knee` 와 무관해야 한다 — 볼이 자국 모양을 정한다는 §2.3b 의 논리.
...
힘 쪽 지표(3 시드 스윕의 Fz·전단 최소/포화 해상도/해상도 손실)를 42 쌍으로 본 것.
...
| `lev_16` | 전단 포화 해상도 | 16 | −0.48 | 0.058 | 0.36 |
...
"형상 포화 해상도 = 힘 포화 해상도" 같은 일대일 대응은 보이지 않는다(형상 `fail_at` 은 대부분 8×5, 힘 포화 해상도는
16–80 px).



### 3.2 파생 변수 × 성능 (`correlations.py`, **53 / 53 유닛**: 9DTact 17 + DIGIT 18 + DIGIT_Marker 18)

![correlations](figures/correlations_pooled.png)
![hypotheses](figures/hypotheses.png)

**2026-09-11 갱신: 표본이 23 → 53 으로 늘면서 결론의 절반이 바뀌었다.** 아래는 새 숫자이고,
바뀐 항목은 그렇게 표시했다.

**세 원리를 그냥 합치면 거의 모든 것이 "유의"해진다.** 원리를 통제한 편상관(원리별 평균 순위
제거)에서 150 쌍 중 **4 개만** 남고, **전부 전단 바닥에 대한 것**이다:

| x | y | n | 편상관 ρ | q |
|---|---|---|---|---|
| `lat_rate_per_frame` (프레임당 전단 라벨 변화) | 전단 최소 | 53 | **+0.685** | <0.001 |
| `lat_noise_mae` (전단 라벨의 2 차 차분) | 전단 최소 | 53 | **+0.676** | <0.001 |
| `fz_noise_mae` | 전단 최소 | 53 | **+0.612** | <0.001 |
| `fz_rate_per_frame` | 전단 최소 | 53 | **+0.522** | 0.002 |

**F/T 센서 자신의 잡음(`lat_sd_0N`)은 여전히 무관하다**(+0.042, p 0.77). 결론은 23 유닛
때와 같고 더 강해졌다: **전단 추정의 바닥을 정하는 것은 센서 잡음이 아니라 "한 프레임 안에서
힘이 얼마나 움직이는가"** 다. 5 fps 카메라의 200 ms 노출 동안 힘이 움직이면 그 프레임의
라벨은 어느 순간의 힘인지 정의되지 않고, 그 크기가 그대로 바닥이 된다. `fz_noise_mae` 와
`fz_rate_per_frame` 까지 **전단** 바닥을 예측한다는 것은 이 바닥이 이미지가 아니라 **라벨의
성질**임을 다시 가리킨다.

**원리 안에서만 보면** 9DTact 2 개(같은 두 변수 → 전단 최소, ρ +0.80 / +0.71),
DIGIT 1 개, DIGIT_Marker 0 개가 q < 0.10 을 넘는다. DIGIT 의 그 하나는 새 결과다 —
**`mu` → 전단 포화 해상도 ρ −0.820, q 0.005**(18 유닛, 150 검정). 마찰이 큰 유닛일수록 전단 포화 해상도가
낮은 크기에서 온다. 포화 해상도를 예측하는 변수가 하나라도 나온 것은 이번이 처음이고, 한 원리
안에서만 나온 것이라 재현이 필요하다.

**가설 판정** (편상관 = 원리 통제, n = 53):

| | 판정 | 근거 |
|---|---|---|
| H1 응답률 → Fz 바닥 | **철회 — 기각** | 23 유닛에서 ρ −0.41 p 0.055 였으나 53 유닛에서 **ρ −0.12 p 0.39** |
| H2 라벨 잡음 → 바닥 | **재해석하여 지지 (강화)** | 센서 잡음은 무관, **프레임당 라벨 변화**가 전단 바닥을 정한다 (ρ +0.69, q < 0.001) |
| H3 대역폭 → 포화 해상도 | **기각** | `bw_shear_f90` → 전단 포화 해상도 +0.10 p 0.50 |
| H4 접촉 반경 → Fz 포화 해상도 없음 | **예측대로** | `bw_normal_f90` → Fz 포화 해상도 +0.17 p 0.22 |
| H5 강성 → 전단 해상도 손실 | **기각** | `hertz_k` +0.07 p 0.61 |
| H6 그림자 → Fz 바닥 | **예측대로 무관** | `pedestal_ratio` −0.05 p 0.74 |
| H7 2 N 응답 → Fz 바닥 | **철회 — 기각** | 23 유닛에서 ρ −0.50 **p 0.015** 였으나 53 유닛에서 **ρ −0.05 p 0.71** |

**H1 과 H7 의 철회가 이 절에서 가장 중요한 변화다.** "뉴턴당 이미지 응답이 Fz 바닥을 정한다"
는 23 유닛에서 가장 그럴듯한 설명이었고, 표본을 두 배로 늘리자 남지 않았다. 그때 그 상관을
만든 것은 **DIGIT_Marker 6 유닛**이었을 가능성이 크다(그 여섯이 9DTact 17 보다 응답이 낮고
동시에 Fz 바닥이 높았다) — 즉 원리 통제가 6 유닛으로는 충분하지 않았다.

**그래서 이 표의 파생 변수 중 `fz_best` 를 예측하는 것은 하나도 없다.** 다만 §3.4 가
노출 창 안의 라벨 이동을 **직접 재서** 예측한다(`blur_fz_N` → `fz_best` ρ +0.478,
p 0.0003, 53 유닛). 즉 Fz 바닥도 라벨로 설명되기는 하는데, 그것을 보려면 스윕에서 뽑은
대리 변수가 아니라 창 안 이동을 직접 재야 한다.

**자기비판.** (i) `lat_rate_per_frame` 과 `lat_noise_mae` 는 거의 같은 것을 잰다; 독립 증거가
아니다. (ii) 성능 지표의 시드 수가 원리마다 다르다(9DTact 1–3, DIGIT·Marker 3). (iii) 해상도
**포화 해상도**을 예측하는 변수는 원리 통제 후에도 없다(DIGIT 안의 `mu` 하나 제외). (iv) 이 절의
결론이 23 → 53 에서 절반 바뀌었다는 사실 자체가, 남은 결론도 표본이 더 늘면 바뀔 수 있음을
뜻한다.
...
### 3.4 힘 바닥의 오차 예산: 라벨이 어디까지 설명하나 (**53 유닛**, 2026-09-11 갱신)
...
**결과 1 — 창 안 이동은 학습된 바닥을 예측한다.** 원리 통제 순위 편상관, **n = 53**
(괄호는 23 유닛 때의 값):

| 대상 | 창 안 Fz 이동 | 창 안 전단 이동(p90) | 2 N 이미지 응답 |
|---|---|---|---|
| `fz_best` | **+0.478** (p 0.0003) *(+0.63)* | +0.277 (p 0.044) *(+0.45)* | **철회** *(−0.46)* |
| `sh_best` | **+0.504** (p 0.0001) *(+0.68)* | **+0.654** (p 0.000) *(+0.69)* | 유의하지 않음 |
| `fz_knee` / `sh_knee` | 없음 | 없음 | 없음 |

**표본을 두 배로 늘려도 살아남았다** — 상관 자체는 약해졌지만(0.63 → 0.48) p 는 오히려
낮아졌다. 같은 표의 "2 N 이미지 응답" 열은 §3.2 와 함께 **철회한다**(53 유닛에서 ρ −0.05,
p 0.71). 창 안 라벨 이동은 남고, 이미지 응답은 남지 않았다.

9DTact 안에서만 봐도(n = 17, 원리 혼입이 아예 없다) `fz_best` +0.62 (p 0.008), `sh_best` +0.78 (p 0.000) 로
같은 방향이다. 포화 해상도 위치는 여전히 아무 것도 예측하지 못한다 — 바닥과 포화 해상도는 서로 다른 것이 정한다.
...
### 3.5 마커 격자가 접촉보다 굵다 — 그런데 H9 의 예측은 맞았다

> **갱신 2026-09-13.** 아래 §3.5 는 "H9 은 이 데이터로는 시험할 수 없다" 로 끝난다.
> 그 뒤 세 원리를 1920×1080 부터 8×5 까지 **학습 파라미터를 고정한 채** 다시 학습해
> 축별로 보니(§3.5a) **H9 의 예측이 그대로 맞았다.** 격자가 굵다는 사실은 그대로이고,
> 예측이 맞은 기전이 H9 이 상정한 것과 다를 뿐이다.
...
> **정정 2026-09-13.** 아래 표는 처음에 점 간격을 **3.00 mm** 로 적었다. 틀렸다 —
> 운전자가 실물을 재서 **점 지름 1.0 mm, 중심 간격 2.5 mm** 임을 확인했고, 설계
> 사양도 그렇다(`campaign_protocol.md` §5). 3.00 mm 는 `pitch_px / px_per_mm` 로
> **계산한** 값이었고, 그 `px_per_mm` 이 20 % 낮았던 것이다. 자세한 것은 §3.5b.

| 양 | 중앙값 | 범위 |
|---|---|---|
| 점 간격 | **2.50 mm** (284 px) — 실측 확인 | 설계값 |
| 점 지름 | **0.88 mm** (97 px) | 0.65–0.96 mm (공칭 1.0) |
| 2 N 접촉 지름 (ball8, Hertz — 축척과 무관) | **3.78 mm** | 2.79–4.61 mm |
| 접촉 지름 / 점 간격 | **1.53** | 1.11–1.84 |
| 접촉 안의 점 개수 | **1.83** | 0.98–2.67 |

**접촉 하나에 점이 두 개 남짓 들어온다.** 그러므로 이 프로브에서 마커 채널이 실어 나르는 것은 변위 "장" 이
아니라 사실상 벡터 한두 개다. 전단의 비대칭을 점 배치로 읽을 표본이 없다 — H9 이 상정한
**기전은 이 프로브에서 성립하지 않는다**(예측 자체는 §3.5a 에서 맞았다. 기전이 다를 뿐이다). 게다가 불투명한 점이 접촉의 약 11 % 를
가리므로, 마커 유닛에서 남는 정보는 점 사이의 명암 압흔이고 그 압흔은 부분적으로 가려져 있다. §4 에서 마커의
Fz 포화 해상도가 9DTact 보다 높게 나오면(잠정 80 대 32 px) 그 원인으로 이 가림을 먼저 의심해야 한다.

**공이 어디에 떨어졌나 (가림의 크기).** 프로브는 매 유닛 같은 자리(≈ 화면 (900, 700))에 내려오지만 겔을 다시
장착할 때마다 격자의 위상이 달라진다. 접촉 중심에서 가장 가까운 점까지 50–142 px
(간격의 0.19–0.46 배)이고, **접촉 원의 8.9–14.0 %(중앙 11.0)가 불투명한 점 아래에
들어간다**(`data/analysis/marker_occlusion.csv`, 2026-09-13 에 격자 축척으로 다시 계산 — §3.5b).
접촉 반지름이 중앙 211 px (174–236)인데 가장 가까운 점이 그보다 가까우니
**모든 유닛에서 접촉이 점과 겹친다** — 겹치지 않게 놓인 유닛이 하나도 없어서, 가림의 유무를 대조할 수는 없고
정도(8.9–14.0 %)만 비교할 수 있다. 마커 스윕이 끝나면 이 값과 유닛별 바닥의 관계를 §4 에서 확인한다.

**해상도 쪽 예측도 뒤집힌다.** 점이 2 px 로 유지되는 폭은 40 px, 간격이 2 px 로 유지되는 폭은
14 px 다. 즉 **격자 자체는 아주 낮은 해상도까지 살아남는다** — 마커 유닛의 높은 포화 해상도는
"점이 안 보여서" 가 아니다. 포화 해상도는 여전히 압흔이 정한다.
...
### 3.5b 점 간격은 3.00 mm 가 아니라 2.50 mm 다 — 축척이 20 % 낮았다 (2026-09-13)

운전자가 마커 겔의 격자를 실물로 쟀다: **점 지름 1.0 mm, 중심 간격 2.5 mm.** 설계
사양과 같다(`campaign_protocol.md` §5). 그런데 `marker_geometry.csv` 는 간격을
**3.00 mm**, 점 지름을 **1.09 mm** 로 내고 있었다.

그 두 값은 잰 것이 아니라 **계산한** 것이다 — `pitch_px / px_per_mm` 이고, 그
`px_per_mm` 은 비마커 DIGIT 18 유닛에서 얻은 **표면 높이 회귀**(`kx = −12.44 x
surface + 403.1`, r −0.994)로 마커 유닛에 외삽한 값이다. 등록부에서 마커 유닛의
`scale_trusted` 가 18 개 중 **15 개에서 False** 인 것이 그 뜻이다(`methods.md` §10.4).

격자를 자로 쓰면(간격 = 2.5 mm) 축척이 이렇게 갈린다:

| | 중앙 | 범위 |
|---|---:|---|
| 표면 높이 회귀 | 91.2 px/mm | 82.8 – 116.9 |
| **격자 (2.5 mm)** | **113.6 px/mm** | 97.6 – 128.0 |
| 비 (격자 / 회귀) | **1.198** | 1.075 – 1.260 |

**잡음이 아니라 계통 오차다** — 18 유닛 전부에서 같은 방향이고 폭이 ±10 % 안이다.

**검산은 점 지름이 한다.** 간격과 지름은 같은 그림에서 나오지만 서로 다른 공정이
만든 길이다(전자는 도안, 후자는 인쇄). 격자 축척으로 지름을 재면 **0.88 mm** 로
공칭 1.0 보다 조금 작은데, 문턱이 무른 가장자리를 깎으므로 **그것이 맞는 방향**이다.
회귀 축척으로 재면 1.09 mm 로 공칭보다 **크게** 나오고, 그럴 이유가 없다.

> **`scale_from_markers.py` 의 보류를 푼다.** 그 스크립트는 2026-09-08 에 이 15 % 차이를
> 보고 "도안이 공칭이 아닐 수 있다 — 7 % 수축이면 실제 간격이 2.33 mm" 라며 값을
> 보고만 하고 쓰지는 않았다. **실측이 2.5 mm 이므로 수축 가설은 죽는다.** 남는 설명은
> 회귀 축척이 낮다는 것이고, 그럴 만도 하다 — 마커가 없는 겔에서 세운 회귀를 마커가
> 있는 겔(표면이 다르고 점이 표면보다 0.5 mm 아래에 있다)에 외삽한 것이다.

**무엇이 영향을 받나.** 마커 유닛의 `px_per_mm` 을 지나가는 양만이다:

| 영향 있음 | 영향 없음 |
|---|---|
| §3.5 의 점 간격·지름·접촉당 점 개수 | **힘 추정 전부** — mm 를 쓰지 않는다 |
| §3.6 의 픽셀 요구량(cycles/px) | **천장** — N 과 깊이는 로봇·F/T 가 준다 |
| `marker_occlusion.csv` 의 가려진 비율 | **광학 곡선** — px 대 깊이 mm 이고 깊이는 로봇이 준다 |

**§3.5 의 결론은 살아남는다.** 접촉당 점이 1.24 개에서 **1.83 개**(0.98–2.67)로 늘지만, 변위
"장" 을 표본화하려면 여전히 턱없이 모자란다. 다만 **숫자가 44 % 틀렸으므로 인용할 때
고쳐 쓸 것.**

### 3.5a H9 판정 — 마커는 전단의 해상도 의존성을 **뒤집는다** (2026-09-13)

> **마커를 남기고 다시 잰 결과 (2026-09-15).** 운전자가 "마커 쓰는 센서들이
> 힘 추정할 때 전처리를 그렇게 안 할 것 같다" 고 지적했다. 맞다 — 마커 기반
> 센서의 표준은 Marker Displacement Method 로 마커를 **추적**해 전단을 읽고,
> FEATS 는 GelSight Mini 원본 이미지를 그대로 넣는다. 아래 판정은 마커 점을
> **지운**(`inpaint`) 표현으로 얻은 것이라, 마커를 남긴 `colour`·`grey` 로
> 선택 아홉 유닛을 다시 학습했다. 같은 학습 판에서 축분리 전단으로:
>
...
>
> **방향은 살아남는다.** 마커 없는 두 팔은 전해상도에서 34 ~ 36 % 나빠지는데
> 마커 팔은 어느 전처리에서도 나빠지지 않는다(비 0.85 ~ 0.95). 그러므로
> **H9 의 뒤집힘은 마커를 지운 탓이 아니다.**
>
> **세기는 살아남지 않는다.** 지우면 전해상도가 15 % **좋아지고**(0.85), 남기면
> 5 ~ 6 % 로 사실상 평평하다(0.94 ~ 0.95). 유의성도 p 0.039 에서 0.30 ~ 0.36
> 으로 사라진다. 즉 정확한 진술은 **"마커는 전단이 고해상도에서 나빠지는 것을
> 막는다"** 이지 "고해상도에서 좋아지게 한다" 가 아니다. 뒤의 문장은 inpaint
> 에서만 성립한다.
>
> 아래 표의 "15/18, p 0.002" 는 18 유닛 · inpaint 판이다. 마커를 남긴 학습은
> 선택 아홉 유닛만 돌렸으므로 18 유닛으로는 견줄 수 없다.
> 자료: `force_vs_resolution_{colour,grey}_axes.csv`.

세 원리를 12 단(1920×1080 ~ 8×5)에서 다시 학습했다. **학습 파라미터는 원리·해상도에
걸쳐 전부 같다** — 실효 배치 64(경사 누적으로 고정), 30 에폭, Adam 5e-4, cycle 분할,
seed 3 개. 전단 = (Fx MAE + Fy MAE) / 2 의 유닛 중앙값:

| 해상도 | 9DTact | DIGIT | DIGIT_Marker |
|---|---:|---:|---:|
| 8×5 | 0.0450 | 0.0608 | 0.0703 |
| **48×27** | **0.0262** | **0.0304** | 0.0380 |
| 80×45 | 0.0272 | 0.0312 | 0.0337 |
| 320×180 | 0.0283 | 0.0311 | 0.0376 |
| 1280×720 | 0.0301 | 0.0348 | 0.0341 |
| **1920×1080** | 0.0356 | 0.0391 | **0.0297** |

| | 해상도–오차 상관 | 포화 해상도 | 최소 |
|---|---|---|---|
| 9DTact | ρ +0.084 p 0.80 (평평) | 48×27 | 48×27 |
| DIGIT | ρ −0.049 p 0.88 (평평) | 48×27 | 48×27 |
| **DIGIT_Marker** | **ρ −0.755 p 0.005** | **1920×1080** | 1920×1080 |

유닛 단위 대응 검정(Wilcoxon)에서 **방향이 뒤집힌다**:

| | 1920×1080 이 80×45 보다 나은 유닛 | p |
|---|---|---|
| 9DTact | 3 / 17 | 0.001 |
| DIGIT | 1 / 18 | < 0.001 |
| **DIGIT_Marker** | **15 / 18** | 0.002 |

**H9 이 예측한 그대로다.** 마커 없는 두 원리는 전해상도에서 오히려 나빠지고(과적합
팔), 마커가 있으면 좋아진다.

**그런데 기전은 H9 이 상정한 것이 아닐 것이다.** §3.5 가 잰 것은 그대로다 — 접촉 하나에
점이 1.83 개뿐이라 변위 "장" 을 표본화하지 못한다. 그런데도 해상도가 도움이 된다면,
남는 설명은 **점 하나의 위치를 더 정밀하게 읽는 것**이다. 점 개수가 아니라 점 하나의
중심 추정 정밀도가 해상도를 따라 좋아지고, 전단 벡터 하나의 정밀도가 그만큼 올라간다.
이 해석은 이 데이터로 검증되지 않았다 — **예측이 맞은 것과 기전이 맞은 것은 다르다.**

> **효과 크기는 작다.** 마커의 1920 대 80 중앙 차이는 −0.0036 N, 0.033 N 대비 11 %
> 이고 seed 3 개의 폭(0.0054 N)보다 작다. 유닛 하나로는 구별되지 않는다. 근거는
> **18 유닛 중 15 개가 같은 방향**이라는 것이다. 방향은 믿을 만하고 크기는 그렇지 않다.

> **Fz 는 반응하지 않는다.** 세 원리 모두 80×45 에서 바닥친다. 전단만 마커에 반응하고,
> 이것은 마커가 **접선 변위**를 싣는다는 설명과 일관된다.
...
1. **원리가 요구량의 자릿수를 정한다.** 9DTact 8–20 px, DIGIT 17–44 px, DIGIT_Marker 24–105 px. 9DTact 의
   ...
   점의 가장자리가 스펙트럼을 지배한다(§3.5). 측정된 포화 해상도의 순서(9DTact 32/48 px, 마커 80/120 px)와 방향이
...
3. **경도는 어느 원리에서도 요구량을 바꾸지 않는다**(모든 p > 0.21). 포화 해상도가 경도를 따르지 않았던 §2.3b 의
   결과와 일치한다.

즉 "겔 두께가 포화 해상도를 바꾼다" 는 **DIGIT 계열에서만 성립하고 9DTact 에서는 성립하지 않는다**. 이는
스윕이 끝나면 유닛별 포화 해상도로 직접 확인할 수 있는 예측이다(§4).
...
> **주의 2026-09-13.** 그 회귀 축척이 **20 % 낮다**는 것이 §3.5b 에서 확인됐다. 아래
> 숫자들은 아직 회귀 축척으로 낸 것이므로 **격자 축척으로 다시 내면 20 % 움직인다**
> (방향은 바뀌지 않는다 — 모든 마커 유닛에 같은 배율이 걸린다).
...
**예측을 포화 해상도와 맞춰보면: 원리 사이는 맞고 원리 안은 맞지 않는다.** 9DTact 17 유닛에서 예측 폭과 측정된
포화 해상도의 순위 상관은 법선 ρ −0.48 (p 0.052), 전단 ρ −0.18 (p 0.50) 으로 **양의 관계가 없다**. 측정된 포화 해상도는
...
(0.2/0.3/0.4)뿐이고, 포화 해상도 자체가 2 배 간격의 크기 사다리에서 읽히므로 한 칸이 곧 2 배다. 그러므로 §3.6 의
두께 결론은 **그림이 요구하는 양에 대한 진술**이지 아직 학습 포화 해상도에 대한 진술이 아니다 — DIGIT 스윕이
끝나면 그 예측(24 → 19 → 17 px)이 포화 해상도에서도 보이는지 §4 에서 확인한다.
...
(pair010 p 0.38–0.78, pair025 p 0.07–0.88). §3.6(대역폭), §3.9(광학), §3.10(포화 해상도)과 같다.
...
### 3.10 DIGIT 스윕이 완성됐다: 전단 포화 해상도가 두께를 따라 움직인다 (18 유닛 × 12 크기, 1 seed)

캠페인의 원래 질문 — "오차가 포화하는 해상도가 겔에 따라 달라지는가" — 에 대해 **DIGIT 에서 처음으로
학습 결과가 그렇다고 답한다**. 전단 포화 해상도의 두께별 중앙값:

| 두께 | 전단 포화 해상도 | Fz 포화 해상도 | 전단 바닥 | Fz 바닥 |
|---|---|---|---|---|
...
**두꺼운 겔일수록 더 낮은 해상도에서 포화한다**(160 → 80 → 64 px). 경도는 전단 포화 해상도에서 무효다(ρ +0.03).
...
| §3.10 전단 포화 해상도 | 오차가 바닥을 뜨는 크기 (학습) | 예 | 160 → 64 px | 두꺼울수록 덜 든다 |

물리적으로 하나의 이야기다: **두꺼운 DIGIT 겔은 압흔을 옆으로 퍼뜨리는 저역통과 필터**이고, 저역통과된
그림은 더 적은 화소로 담긴다. 9DTact 에서는 이 사슬이 끊긴다 — 그 압흔은 이미 8–12 px 면 담기므로
(§3.6) 더 흐려질 여지가 없고, 실제로 9DTact 의 포화 해상도는 모든 겔에서 48 px 였다(§2.3b).

**Fz 는 다르게 움직인다.** 포화 해상도는 두께를 따르지 않고(ρ −0.16), 대신 **해상도 손실**(8×5 − 최저)이 두께를
...
| 주장 | 1 seed | 3 seed | 판정 |
|---|---|---|---|
| DIGIT 전단 포화 해상도 ~ 두께 | ρ −0.466 (p 0.051) | ρ **−0.456** (p **0.057**) | **그대로** — 값도 유의성도 거의 안 움직였다 |
...
seed 산포를 재지 않은 p 값이었다. 포화 해상도는 2 배 간격 사다리에서 읽히므로 seed 잡음에 둔감하고, 해상도 손실은
...
경도를 따르는 결과가 재현된다(ρ **+0.500**, p **0.041**; 옛 분할에서는 +0.49, p 0.048). 전단 포화 해상도는 두께·경도
모두에 평평하다(경도별 120/48/48 px, 두께별 64/48/64) — §2.3b 의 "모든 겔에서 48 px" 가 분할을 고쳐도 유지된다.

**표현을 바꾸면 결과가 바뀐다 (2026-09-11).** 위 검정은 `grey` 스윕에서 한 것이다. 같은
유닛·같은 3 seed 를 두 표현으로 각각 재면:

| 원리 / 표현 | 전단 포화 해상도 두께별 (1/2/3 mm) | ρ | p |
|---|---|---|---|
| DIGIT / `grey` | 120 / 32 / 48 px | **−0.456** | **0.057** |
| DIGIT / `colour` | 48 / 48 / 40 px | +0.014 | 0.957 |
| DIGIT / `colour + ImageNet` (1 seed) | 80 / 240 / 237 px | −0.053 | 0.834 |
| DIGIT_Marker / `grey` | — | −0.162 | 0.520 |
| DIGIT_Marker / `colour` | — | **−0.609** | **0.0073** |
| 9DTact / `grey` | — | −0.082 | 0.754 |

**여섯 중 둘이 유의한데, 유의한 쪽이 원리마다 다른 표현이다.** DIGIT 은 grey 에서만
나오고 colour 계열 두 표현에서는 없다(+0.014, −0.053); 마커는 반대로 colour 에서 나온다.
같은 프레임·같은 학습 조건이고 입력 표현만 다르다.

읽을 수 있는 것과 없는 것을 갈라 두면:

- **방향은 일관된다.** 여섯 검정의 ρ 가 모두 0 이하다(−0.61 … +0.01). 두꺼운 겔이 더 작은
  크기에서 포화한다는 방향은 한 번도 뒤집히지 않았고, §3.6(압흔 대역폭 24 → 17 px)과
  §3.9(광학 번짐 1.0 → 3.2 mm)의 학습 없는 두 측정과도 같은 방향이다.
- **크기와 유의성은 재현되지 않는다.** 어느 표현에서 유의해지는지가 원리마다 다르다는 것은,
  이 포화 해상도 추정이 그 차이를 견딜 만큼 안정적이지 않다는 뜻이다.

포화 해상도를 2 배 간격 사다리에서 읽는 것이 여기 한몫한다 — 120 px 와 48 px 는 두 칸 차이일
뿐이다. 그래서 **√2 간격 사다리로 다시 물었다**(16, 24, 32, 40, 48, 64, 80, 113, 160,
226, 320 px; `colour`, 3 seed, `sqrt2_knee.py`).

| 전단 포화 해상도 ~ 두께 | 1 mm | 2 mm | 3 mm | ρ | p |
|---|---|---|---|---|---|
| DIGIT, 2 배 사다리 | 48 px | 48 px | 40 px | +0.014 | 0.957 |
| **DIGIT, √2 사다리** | **40** | **56** | **40** | **−0.047** | **0.854** |
| DIGIT_Marker, 2 배 사다리 | 400 px | 854 px | 48 px | **−0.609** | **0.0073** |
| **DIGIT_Marker, √2 사다리** | **44** | **36** | **44** | **−0.113** | **0.656** |

**두 원리 모두 사라진다.** 마커에서 유일하게 유의했던 −0.609 는 포화 해상도가 400 / 854 / 48 px
로 벌어진 위에서 계산된 값이고, √2 사다리에서는 44 / 36 / 44 px 로 좁고 평평해지면서
−0.113 (p 0.656) 이 된다. **불안정한 포화 해상도 추정이 만든 허상이었다.**

포화 해상도가라는 양 자체는 견고하다 — DIGIT 40–56 px, 마커 36–44 px 로 2 배 사다리의 48 px
근처에 있다. 흔들린 것은 포화 해상도의 값이 아니라 **그것이 겔을 따라간다는 주장**이다.

**그래서 "포화 해상도가 겔에 따라 달라지는가" 에 대한 이 캠페인의 답은 '아니오' 다.**
방향(모든 ρ ≤ 0)과 학습 없는 두 측정(§3.6 압흔 대역폭 24 → 17 px, §3.9 광학 번짐
1.0 → 3.2 mm)은 그대로 남지만, **학습된 포화 해상도로는 이 효과를 보일 수 없다.**

**√2 사다리에서 새로 나온 두 신호는 기록만 한다.** 마커의 전단 포화 해상도가 경도를 따르고
(72 / 40 / 36 px, ρ −0.591, **p 0.010**), Fz 포화 해상도가 두께를 따른다(28 / 40 / 44 px,
ρ +0.542, **p 0.020**). 둘 다 **마커에만 있고 DIGIT 에는 없다**(각각 p 0.56, p 0.40).
원리 하나·조건 하나에서만 나오는 이 모양이 이 캠페인에서 세 번 철회된 주장들과 같은
모양이므로(§5.1), 재현 전까지는 결과로 세지 않는다. 여덟 검정(2 원리 × 2 채널 × 2 변수)
중 둘이 p < 0.05 인 것은 우연이 주는 0.4 보다는 많지만 확정할 수 있는 양이 아니다.

포화 해상도를 2 배 간격 사다리에서 읽는 것도 이 취약함에 한몫한다 — 한 칸이 곧 2 배이므로
120 px 와 48 px 는 두 칸 차이일 뿐이다. √2 간격 사다리(`data_wishlist.md` #10)가 있어야
이 질문에 제대로 답할 수 있다.
...
**이것이 경도가 모든 분석에서 무효였던 이유일 수 있다.** 힘 추정 포화 해상도(§3.10), 압흔 대역폭(§3.6), 겔 광학
...
경고했던 마커 축척이 실제로는 쓸 만하다는 세 번째 증거다(점 격자 2.50 mm, pair 자국 간격에 이어. §3.5b 에서 그 격자가 회귀 축척보다 20 % 높음이 확인됐다).
...
`force_estimation.md` §8 의 차이 중 이것 하나는 데이터로 판정됐다.

`raw` 는 두 가지를 동시에 바꾸므로(차분 제거 + 정규화) `colour + ImageNet` 팔을 따로
돌려 갈랐다: **정규화는 무관하고**(Fz p 0.97, 전단 p 0.52) **기준 차분이 전부다**
(정규화를 켠 채 차분만 빼면 전단 p 0.0008). `force_estimation.md` §7.3b.
...
- **전단의 오차 수준은 원리가 갈리지 않는다.** 0.0432–0.0523, 모든 쌍 p ≥ 0.33.
  마커를 붙인다고 전단이 더 정확해지지는 않는다.

  > **H9 과 혼동하지 말 것 (2026-09-13).** 여기서 비교한 것은 전해상도에서의 **오차
  > 수준**이고, H9 이 예측한 것은 **해상도에 따른 곡선의 모양**이다. 둘은 다른 진술이고,
  > 답도 다르다 — 수준은 갈리지 않지만 **모양은 갈린다**(§3.5a: 마커만 1920×1080 까지
  > 내려가고 나머지 둘은 U 자로 돌아선다, 18 유닛 중 15 개가 같은 방향, p 0.002).
  > 이 자리에 "H9 은 전해상도에서 성립하지 않는다" 고 적었던 것은 **가설이 말하지 않은
  > 것을 반증했다고 적은 셈**이라 고친다.
...
전단 포화 해상도(오차가 전해상도의 1.5 배를 넘는 최대 크기)의 유닛 중앙은 9DTact 8 px,
DIGIT 8 px, **DIGIT_Marker 16 px** 다. 마커 쪽이 픽셀을 두 배 요구한다 — §3.5 의
마커 격자 굵기(피치 2.50 mm, 접촉 3.78 mm — §3.5b 에서 3.00 을 정정)와 같은 방향이다. 다만 유닛별 범위가
8–1280 px 로 넓어(§5) 중앙 말고는 읽지 않는다.

**화소 밀도로 옮기면 차이가 더 벌어진다.** 같은 픽셀 폭이 원리마다 다른 물리 밀도다 —
1920 px 폭에서 9DTact 7 830, DIGIT 10 389, DIGIT_Marker 12 905 px/mm² 이고, 밀도는
폭의 제곱에 비례한다(R = (s·k)², `pixel_density.py`). 위 세 값은 **0.14 / 0.18 /
0.90 px/mm²** 가 된다 — 폭으로는 2 배지만 밀도로는 **5~7 배**다. 마커 카메라의 시야가
더 좁아 같은 폭이 이미 더 촘촘하기 때문이다.

## 4.9 필요 해상도는 원리가 정하고 겔은 못 바꾼다 — 2026-09-13

축별 재학습(`force_estimation.md` §2.6)이 이 캠페인의 표제 질문에 답한다.
포화 해상도는 **최소 MAE 의 110 % 안에 드는 가장 낮은 해상도**로 정의한다.

| 축 | 9DTact 포화 해상도 | DIGIT 포화 해상도 | p | 9DTact 최소 MAE | DIGIT | p |
|---|---:|---:|---|---:|---:|---|
| Fx | 48 px · R 4.9 | 48 px · R 6.5 | 0.758 | 0.0229 N | 0.0232 | 0.754 |
| **Fy** | **80 px · R 13.6** | **40 px · R 4.5** | **0.003** | **0.0237** | 0.0310 | **0.020** |
| **Fz** | **160 px · R 54.4** | **64 px · R 11.5** | **0.015** | **0.0416** | 0.0557 | **0.00005** |

<sub>R 은 화소 밀도(px/mm²) — 그 폭에서의 총 화소수를 그 카메라가 보는 면적으로 나눈
값. **p 는 픽셀 폭으로 돌린 검정이다**(밀도로 다시 돌리지 않았다). 원리 수준에서
DIGIT 의 밀도가 9DTact 의 1.33 배로 **일정하게** 높으므로 순위는 Fx 에서만 뒤집히는데,
그 축에서는 애초에 차이를 주장하지 않았다(p 0.758).</sub>

**두 지표가 반대 방향이다.** DIGIT 은 적은 픽셀로 자기 한계에 빨리 도달하지만 그
한계가 더 높다. 9DTact 는 픽셀을 더 요구하는 대신 더 정확하다. **Fx 에서는 둘 다
차이가 없다** — 축을 밝히지 않고 "A 가 B 보다 해상도를 덜 요구한다" 고 쓰면 안 된다.

**겔은 포화 해상도를 못 가른다**: Fz 포화 해상도 대 경도 ρ +0.10 (p 0.55), 대 두께 ρ −0.06 (p 0.72).
이것은 천장과 **다른 그림이다** — 천장은 9DTact 에서 겔 설계로 2.6 배 범위를 조절할 수
있다(`force_ceiling.md` §6.8). **무엇이 겔에 딸리고 무엇이 원리에 딸리는지가 지표마다
다르다**는 것이 이 캠페인의 결론 하나다.

> `DIGIT_Marker` 는 2026-09-13 기준 학습 중이라 이 표에 없다.
...
- **"천장이 두께를 따라간다"** — hard 2 mm 와 3 mm 만 보고 적었다. 1 mm 쌍이
  9.85 / 5.19 N 으로 들어오면서 무너졌다. (2026-09-12: 9DTact 를 전수로 재면서 **같은
  실수를 다시 했다** — hard 여섯 개만 보고 `천장 = 9.64 + 17.27 × 두께`, R² 0.994 를 적었는데
  soft·medium 이 들어오자 평평해져 전체 R² 가 0.88 로 떨어졌다. `force_ceiling.md` §6.5c.
  **여섯 번째 같은 실수다.**)
...
**다섯 번째는 통제를 빼먹은 것이다.** "마커가 측정 범위를 30 % 넓힌다 (p 0.0007)" 라고
적었다. 천장은 그 유닛의 최대 이미지 응답과 ρ −0.730 으로 얽혀 있는데(포화를 자기 최대의
15 % 로 판정한다) 마커 겔은 응답이 약하다. 통제하면 ρ −0.039 (p 0.82) 로 사라진다
(`force_ceiling.md` §4.1). **측정의 정의가 만든 상관을 센서의 성질로 읽었다.**

**네 번째는 반대 방향이었다.** "전단 재수집(`data_wishlist.md` #2)은 이미 48/53 유닛이
마찰 한계를 넘었으니 불필요하다"고 적었다가, 다시 계산하니 **53 중 2 개**였다. 유닛 이름이
원리 간에 겹치는데(`hard_1mm_r1` 이 세 원리에 모두 있다) `unit` 하나로 결합해 서로 다른
센서의 μ 와 전단을 섞었다. 실제 여유는 9DTact 0.88, DIGIT 0.63, DIGIT_Marker 0.48 이고
**항목은 유효하다** — 잘못된 계산으로 할 일을 지울 뻔했다.
...
- **천장은 "더 멀리 읽는다"와 "원래 덜 반응한다"를 못 가른다 — 이제 정량화됐다.** 포화를
  그 유닛 자신의 최대 응답 대비 15 % 로 판정하므로, 천장과 최대 응답이 **ρ −0.730
  (p < 0.0001)** 로 얽힌다. 마커 겔은 응답이 약하고(중앙 2.11 대 4.22), 최대 응답을
  통제하면 원리 차이가 ρ +0.540 (p 0.0007) → **ρ −0.039 (p 0.82)** 로 사라진다.
  이 절에 "마커가 범위를 넓힌다" 고 적었던 것은 그래서 **철회한다**
  (`force_ceiling.md` §4.1).
  **2026-09-12 갱신 — 부분적으로 해소됐다.** 문턱을 유닛 자신의 피크가 아니라 **고정
  lvl/N** 으로 두면 이 얽힘이 정의상 사라진다. 두 원리를 같은 프로브(`ball8`)로 다시 재고
  절대 기준으로 계산하니 9DTact 대 DIGIT 이 잘림 적은 두 기준에서 **4.3 배(p 8.8e-07)와
  6.6 배(p 2.4e-03)**, 순위도 같다(`force_ceiling.md` §5.1, §6.8). 상대 기준으로는 1.8 배로
  **압축된다** — 응답이 약한 유닛의 천장이 부풀기 때문이고, 복제 쌍 다섯에서 모두 그
  방향이었다(§6.5c). **원리 간 비교에는 절대 기준을 쓴다.** 다만 `DIGIT_Marker` 는 아직
  `ball4` 뿐이라 이 해소가 적용되지 않는다.
