...
- **천장은 "더 멀리 읽는다"와 "원래 덜 반응한다"를 못 가른다.** 포화를 그 유닛 자신의 최대
  ...
  데서 나온 값이기도 하다.
- **광도 보정은 유닛당 격자가 하나뿐이다.** 복제 쌍의 셋 중 하나가 0 근처인데(§5.2), 이것이
  겔의 성질인지 측정의 잡음인지 가를 자료가 없다. 같은 유닛을 두 번 잰 것은 autoframe
  기하뿐이고(회전 30.2°/30.1°, 배율 96/96 px/mm — 재현성은 좋다) 격자 응답까지는 비교하지
  않았다.
